...
privacy: {
  title: 'Chúng tôi cam kết bảo mật', // Our commitment to privacy
  paragraph_one:
  ...
  chapter_two: {
    ...
    hashing_three: {
      ...
      list_one:
          'Giống như dấu vân tay, các băm là duy nhất. Ngoại trừ một số trường hợp đặc biệt, các băm cho hai thứ khác nhau không bao giờ nên giống nhau.',       // Just like fingerprints, hashes are unique. Barring some exceptional circumstances, the hashes for two different things should never be the same.
      list_two:
          'Các hàm băm là đường một chiều. Bạn không thể kỹ thuật đảo ngược một băm để tìm ra dữ liệu được sử dụng để tạo ra nó.',       // Hash functions are one way streets. You can’t reverse engineer a hash to figure out the data used to make it.
      list_three:
          'Các băm cực kỳ đáng tin cậy theo nghĩa là chúng mang tính xác định. Điều này có nghĩa là bạn có thể băm cùng một phần dữ liệu nhiều lần và bạn sẽ luôn nhận được kết quả giống nhau.',       // Hashes are extremely reliable in the sense that they are deterministic. This means you can hash the same piece of data over and over again and you’ll always get the same result.
      paragraph_one: {
      ...
      paragraph_three:
          'Tìm một băm bắt đầu bằng số không (“0”). Tiếp tục nhập các thứ khác nhau bên dưới cho đến khi bạn tìm thấy một băm mà chúng ta muốn.',             // Find a hash that starts with a zero (“0”). Keep typing different things below until you find a hash that we want.
      tooltip_one: {
    ...
    hashing_five: {
      ...
      paragraph_one:
          'Hãy làm cho nó khó hơn một chút. Hãy thử tìm một băm bắt đầu bằng hai số không ("00").',             // Let’s make it a bit more tricky. Try to find a hash that starts with two zeroes ("00").
    },
    ...
    mining_one: {
      ...
      paragraph_one:
          'Hiện tại, mạng lưới bitcoin yêu cầu các khối phải có băm với mười số không hàng đầu. Hãy bắt đầu thôi!',       // Right now, the bitcoin network requires blocks to have a hash with ten leading zeroes. Let’s get to it!
      paragraph_two:
      ...
      paragraph_three:
          'Mã bạn đã viết trong bài học trước để tính toán các băm lặp đi lặp lại đang chạy.',       // The code you wrote in the previous lesson to compute hashes over and over again is running.
      paragraph_four:
      ...
      paragraph_five:
          'Xem trường nonce tăng dần? Đó là số lượng băm mà bạn đã thử cho đến nay!',       // See the nonce field incrementing? That’s how many hashes you have tried so far!
    ...
    resources: {
      hashing_two: {
        ...
        hash_functions_paragraph:
            'Hàm băm là bất kỳ hàm nào có thể được sử dụng để ánh xạ dữ liệu có kích thước tùy ý thành các giá trị có kích thước cố định. Các giá trị được trả về bởi hàm băm được gọi là giá trị băm, mã băm, tóm tắt hoặc đơn giản là băm. Các giá trị thường được sử dụng để lập chỉ mục cho một bảng có kích thước cố định được gọi là bảng băm. Việc sử dụng hàm băm để lập chỉ mục cho bảng băm được gọi là băm hoặc định địa chỉ lưu trữ phân tán.',         // A hash function is any function that can be used to map data of arbitrary size to fixed-size values. The values returned by a hash function are called hash values, hash codes, digests, or simply hashes. The values are usually used to index a fixed-size table called a hash table. Use of a hash function to index a hash table is called hashing or scatter storage addressing.
        tip: 'Cứ tiếp tục nhập! Bởi vì thuật toán SHA-256 bất kỳ đầu vào mới nào cũng sẽ tạo ra một băm hoàn toàn ngẫu nhiên ngay cả khi các đầu vào rất giống nhau',         // Just keep typing! Because of the SHA-256 algorithm any new input will result in a completely random hash even if the inputs are very similar
      ...
      scripting_one: {
        ...
        tip_one:
            'Đảm bảo bạn đang giải mã chính xác các đầu vào. Các thuật toán băm thường trả về các đối tượng được giải mã bằng byte, nhưng chúng ta muốn có thể đọc nó ở định dạng hex!',         // Ensure you are properly decoding the inputs. The hashing algorithms often return objects decoded in bytes, but we want to be able to read it in hex format!
        tip_two:
      ...
      mining_one: {
        ...
        mining_paragraph:
            "Khai thác là nơi tất cả các khái niệm này kết hợp lại. Khai thác Bitcoin là quá trình tạo ra bitcoin mới và thêm các giao dịch vào blockchain. thợ đào cạnh tranh để giải các câu đố toán học phức tạp bằng cách tìm một 'nonce' duy nhất, hợp lệ trong mỗi khối giao dịch. Quá trình bằng chứng công việc này đòi hỏi sức mạnh tính toán và năng lượng đáng kể, biến nó thành một cách an toàn và phi tập trung để xác thực giao dịch. thợ đào thành công được thưởng bằng bitcoin mới được khai thác và phí giao dịch, và họ đóng một vai trò quan trọng trong việc duy trì tính toàn vẹn của mạng lưới bitcoin.",         // Mining is where these concepts all come together. Bitcoin mining is the process by which new bitcoins are created and transactions are added to the blockchain. Miners compete to solve complex mathematical puzzles by finding a unique, valid 'nonce' in each block of transactions. This proof-of-work process demands significant computational power and energy, making it a secure and decentralized way to validate transactions. Successful miners are rewarded with newly minted bitcoins and transaction fees, and they play a critical role in maintaining the integrity of the bitcoin network.
        difficulty_heading: 'Độ khó', // Difficulty
        difficulty_paragraph:
            'Độ khó khai thác là yếu tố cho phép khối duy trì ở mức trung bình 10 phút giữa mỗi khối mới.',         // The mining difficulty is what allows the block to remain at an average of 10 minutes between each new block.
        spoiler:
            'Không có gì để tiết lộ! Chỉ cần tiếp tục đọc bài học này. Sẽ có nhiều cơ hội hơn để thể hiện kiến ​​thức khai thác của bạn trong các bài học trong tương lai!',         // Nothing to spoil! Just go ahead and read through this lesson. There will be more opportunities to show off your  mining knowledge in future lessons!
      },
    },
  },
  chapter_three: {
    ...
    paragraph_one:
        'Bạn nhập địa chỉ của trình khám phá khối và thấy rằng các khối đã trở lại khoảng thời gian 10 phút. Bằng cách nào đó, Holocat ngủ quên tiếng ồn từ tất cả các ASIC.',     // You enter the address of a block explorer and see that blocks are back to ten minute intervals. Somehow, Holocat sleeps through the noise from all the ASICs.
    paragraph_two:
        'Mèo. Bạn có thể làm gì?', // Cats. What can you do?
    paragraph_three:
        'Tuy nhiên, có điều gì đó không ổn. Các khối trống rỗng và các giao dịch không được xử lý. Bạn đã mắc lỗi? Có thể đây là một sự trùng hợp? Một thông báo khác xuất hiện trên màn hình máy tính, đánh thức Holocat.',     // However, something is off. The blocks are empty, and transactions aren’t processing. Did you make a mistake? Could this be a coincidence? Another message pops up on the computer screen, waking Holocat.
    paragraph_four:
    ...
    solo_one: {
      title: 'Bạn vs. BitRey', // You vs. BitRey
      nav_title: 'Khai thác một mình', // Mining alone
      step_zero_heading: 'Hãy thử', // Let’s give it a try
      step_zero_paragraph_one:
          'Trong khi chờ đợi các thợ đào mà bạn đã tiếp cận, bạn quyết định xem liệu BitRey có thể bị đánh bại một mình hay không. Có lẽ bạn có thể chấm dứt các khối trống mà không cần bất kỳ sự trợ giúp nào. Đây là cách xếp hạng hash của bạn. Bạn nghĩ điều này sẽ diễn ra như thế nào?',             // While you’re waiting for the miners you reached out to, you decide to see if BitRey can be fended off alone. Maybe you can put an end to the empty blocks without any help. Here’s how your hash rate stacks up. How do you think this will go?
      step_one_heading: 'Chúng ta đang đua', // We’re off to the races
    ...
    pool_one: {
      ...
      waiting_screen_paragraph_one: `Các máy đào của bạn trong nhà kho không có cơ hội. Nếu bạn kết hợp sức mạnh băm của mình với các thợ đào khác, bạn có thể giữ vững lập trường chống lại cuộc tấn công của BitRey.`,       // Your miners in the warehouse don’t stand a chance. If you combine your hash power with other miners, you might be able to hold the line against BitRey's assault.
      ...
      continue_button: "Đi thôi", // Let’s go
    },
    ...
    pool_two: {
      ...
      step_zero_paragraph_one:
          'Bằng cách kết hợp sức mạnh băm của bạn với các thợ đào khác, bạn có thể chống lại BitRey không?',       // By pooling your hashrate with other miners, can you stand up to BitRey?
    ...
    coop_one: {
      title: 'Coop vs. BitRey', // Coop vs. BitRey
      nav_title: 'Có điều gì đó không ổn', // Something went wrong
      heading: 'Có điều gì đó không ổn.', // Something is not quite right yet.
      paragraph_one:
          'Hashrate Hoppers, người có sức mạnh băm lớn nhất, đang tìm thấy tất cả các khối của bạn nhưng những người khác thì không.',             // Hashrate Hoppers, the one with the most hash power, is finding all your blocks but others find nothing.
      paragraph_two:
      ...
      paragraph_three:
          'Bạn quyết định đặt một trình xác định duy nhất trong “extraNonce” khi chuẩn bị dữ liệu khối cho mỗi người để ngăn chặn các nỗ lực trùng lặp.',       // You decide to place a unique identifier in the “extraNonce” when preparing the block data for each of you to prevent duplicate efforts.
    },
    ...
    coop_two: {
      ...
      paragraph_one:
          'Đối với giao thức khai thác Stratum (không phải giao thức bitcoin), giao dịch coinbase cũng có một thứ gọi là “extra nonce”. Khi cung cấp cho thợ đào dữ liệu khối, các pool khai thác chia extra nonce thành hai phần: “extranonce1” và “extranonce2”.',       // For the Stratum mining pool protocol (not bitcoin protocol), the coinbase transaction also has something called the “extra nonce”. When providing miners with the block data, mining pools divide the extra nonce into two parts: “extranonce1” and “extranonce2”.
      paragraph_two:
          'Việc chia extra nonce thành hai phần có một số lợi thế:',       // Splitting the extra nonce into two parts has several advantages:
      list_one: 'Nó ngăn chặn những người tham gia pool thực hiện cùng một công việc.',       // It prevents pool participants from doing the same work.
      list_two:
          'Nó cho phép pool gửi cùng một danh sách giao dịch cho tất cả các thợ đào đang cùng khai thác. Điều đó có nghĩa là thợ đào chỉ cần cập nhật “extranonce2” và không thay đổi các giao dịch được bao gồm trong khối.',       // It allows the pool to send the same transaction list to all miners that are pooling together. That means miners simply have to update “extranonce2” and not change transactions included in the block.
      list_three:
          '“extranonce1” cho phép các pool xác định và xác định đóng góp cổ phần vì mỗi thợ đào đều có “extranonce1” riêng của mình.',       // The “extranonce1” allows pools to identify and determine share contribution because each miner gets their own “extranonce1”.
      paragraph_three:
          'Các thợ đào đóng góp vào một pool cập nhật “extranonce2” và lặp qua nonce trong tiêu đề khối. Nếu họ không tìm thấy giải pháp, họ sẽ lặp lại quy trình với “extranonce2” khác cho đến khi họ tìm thấy.', // Miners who contribute to a pool update the “extranonce2” and cycle through the nonce in the block header. If they don’t find a solution, they repeat the process with a different “extranonce2” until they do.
      paragraph_four: 'Hãy thử lại bằng cách chia sẻ công việc bằng phương pháp này.', // Let’s try again splitting the work using this method.
    },

    coop_three: {
      title: 'Coop vs. BitRey', // Coop vs. BitRey
      nav_title: 'Khai thác cùng nhau', // Mining together
      step_zero_heading: 'Thử lại lần nữa...', // One more time...
      step_zero_paragraph_one:
          'Hãy xem liệu chúng ta có thể chống lại BitRey bây giờ với chiến lược chia sẻ không gian nonce được cải thiện của chúng ta không.',       // Let’s see if we stand a chance against BitRey now with our improved strategy of dividing the nonce space.
      step_two_heading: 'Bạn đã làm được!', // You did it!
      step_two_paragraph_one:
    ...
    split_one: {
      ...
      paragraph_two:
          'Đối với mỗi khối được khai thác, thợ đào nhận được phần thưởng bằng bitcoin. Nếu nhiều người hơn một người đã làm việc trên khối, nhóm thợ đào (pool khai thác) sẽ nhận được một phần thưởng để chia sẻ.',       // For every block that is mined, the miner gets a reward in bitcoin. If more than one person worked on the block, the group of miners (mining pool) gets a reward to share.
      ...
      paragraph_five:
          'Các pool khai thác giải quyết vấn đề này bằng cách theo dõi số lượng giải pháp một phần mà mỗi thợ đào đã tạo ra. Một pool gán cho mỗi thợ đào một độ khó tối thiểu (thấp hơn độ khó của khối) dựa trên sức mạnh băm của họ. Cứ sau vài giây, một thợ đào sẽ tìm thấy một trong những giải pháp dễ dàng hơn đó và thông báo cho pool. Sau đó, pool theo dõi tất cả các cổ phần đã gửi và chia phần thưởng khối tương ứng.',       // Mining pools solve this problem by tracking how many partial solutions each miner has generated. A pool assigns each miner a minimum difficulty (lower than the block difficulty) based on their hash power. Every few seconds a miner finds one of those easier solutions and notifies the pool. The pool then tracks all the submitted shares and splits up the block rewards accordingly.
    ...
    split_two: {
      ...
      step_zero_paragraph:
          'Chúng tôi sẽ chạy lại trận chiến. Lần này chúng ta chỉ tập trung vào pool của chúng tôi, cụ thể là số lượng giải pháp một phần mà mỗi thợ đào tìm thấy.',       // We will re-run the battle. This time we will only focus on the number of partial solutions each miner found.
      ...
      step_three_heading: 'Tỷ lệ phần trăm sức mạnh băm', // Hash rate percentage
      ...
      step_four_paragraph:
          'Khả năng tìm thấy một khối rất thấp vì nó có rất nhiều liên quan đến may mắn. Đặc biệt trong các kịch bản với một số thợ đào cực kỳ mạnh, các thợ đào thực sự nhỏ có thể đóng góp công việc nhưng không bao giờ tìm thấy bất kỳ khối nào.',       // The chance of finding a block is very low since it has a lot to do with luck. Especially in scenarios with some extremely powerful miners, the really small miners may contribute work but never find any blocks. 
      step_five_heading: 'Tỷ lệ phần trăm giải pháp một phần', // Partial solution percentage
      step_five_paragraph:
          'Bây giờ con số này sẽ hoạt động. Các thợ đào báo cáo các giải pháp này cho các vấn đề dễ dàng hơn thường xuyên cho các pool. Điều đó cho phép các pool đo lường khá chính xác lượng công việc mà mỗi thợ đào đã cung cấp.',       // Now this number will work. Miners report these solutions to easier problems regularly to pools. That allows pools to measure fairly accurately how much work each miner has provided.
    ...
    outro_one: {
      ...
      paragraph_one:
          'Tuyệt vời! Bạn và các thợ đào khác đã có thể đẩy lùi cuộc tấn công 51% của Vanderpoole bằng cách hợp lực tài nguyên của mình. Các khối trống không còn được gửi nữa và có vẻ như anh ta đã từ bỏ.',       // Great work! You and the other miners were able to fend off Vanderpoole’s 51% attack by pooling your resources. Empty blocks are no longer being submitted and it looks like he’s given up.
      paragraph_two:
    ...
    resources: {
      solo: {
        hashrate_heading: 'Tốc độ băm khai thác', // Mining Hashrate
        hashrate_paragraph:
            "Tốc độ băm của một thợ đào riêng lẻ đề cập đến sức mạnh tính toán mà họ đóng góp cho quá trình khai thác mạng lưới bitcoin. Đó là tốc độ mà phần cứng khai thác của họ có thể thực hiện các phép tính toán cần thiết để cố gắng khai thác các khối mới. Tốc độ băm của một thợ đào riêng lẻ thường được đo bằng số băm mỗi giây hoặc ở quy mô terahash, một nghìn tỷ băm mỗi giây (TH/s), tùy thuộc vào quy mô hoạt động khai thác của họ và khả năng của phần cứng khai thác của họ.",         // An individual miner's hashrate refers to the computational power they contribute to the bitcoin network's mining process. It is the rate at which their mining hardware can perform the necessary mathematical calculations to attempt to mine new blocks. The hashrate of an individual miner is typically measured in hashes per second, or at scale in terahashes, one-trillion hashes per second (TH/s), depending on the scale of their mining operation and the capabilities of their mining hardware.
      },
      pool: {
        pool_heading: 'Pool khai thác', // Mining Pool
        pool_paragraph:
            'Pool khai thác đóng một vai trò quan trọng trong việc khai thác tiền điện tử, mang lại một số lợi ích chính cho các thợ đào cá nhân. Bằng cách tập hợp sức mạnh tính toán và tài nguyên của nhiều người tham gia, các pool khai thác tăng cường cơ hội khai thác thành công các khối mới, dẫn đến thu nhập dự đoán và nhất quán hơn. Chúng cũng cung cấp một nền tảng cho các thợ đào để truy cập vào thiết bị khai thác tiên tiến và hỗ trợ chuyên gia, giảm bớt rào cản gia nhập và san bằng sân chơi. Hơn nữa, các pool khai thác giúp phân phối phần thưởng hiệu quả, đảm bảo rằng các thợ đào nhận được phần thưởng công bằng cho những đóng góp của họ, làm cho việc khai thác tiền điện tử trở nên dễ tiếp cận, ổn định và mang lại lợi nhuận tài chính cho một cộng đồng người tham gia rộng lớn hơn.',         // A mining pool plays a pivotal role in cryptocurrency mining, offering several key benefits to individual miners. By pooling together the computational power and resources of many participants, mining pools enhance the chances of successfully mining new blocks, resulting in more predictable and consistent earnings. They also provide a platform for miners to access advanced mining equipment and expert support, reducing barriers to entry and leveling the playing field. Furthermore, mining pools help distribute rewards efficiently, ensuring that miners receive their fair share for their contributions, making cryptocurrency mining accessible, stable, and financially rewarding for a broader community of participants.
      },
      coop: {
        distribution_heading: 'Phân phối công việc', // Job Distribution
        distribution_paragraph:
            "Các pool khai thác thực hiện các biện pháp phòng ngừa để đảm bảo rằng những người tham gia của họ không khai thác cùng một nonce cho cùng một khối. Điều này rất quan trọng bởi vì, trong quá trình khai thác bitcoin, các thợ đào cạnh tranh để tìm một nonce hợp lệ trong mỗi khối giao dịch. Nếu hai thợ đào trong cùng một pool làm việc trên cùng một nonce đồng thời, điều đó sẽ không hiệu quả và chỉ một người sẽ nhận được phần thưởng khối. Để ngăn chặn điều này, các pool khai thác sử dụng một quy trình gọi là 'giao việc' hoặc 'phân phối công việc' để phân bổ công việc hiệu quả. Các pool khai thác phân phối công việc cho từng thợ đào hoặc người tham gia. Khi cần khai thác một khối mới, máy chủ của pool (hoặc nhà điều hành pool) tạo ra một 'công việc' duy nhất cho các thợ đào. Công việc này bao gồm tất cả thông tin cần thiết để khai thác khối, chẳng hạn như danh sách giao dịch chưa xác nhận hiện tại, tiêu đề của khối trước đó và độ khó mục tiêu.",                 // Mining pools take precautions to ensure that their participants do not mine the same nonce for the same block. This is important because, in the bitcoin mining process, miners compete to find a unique, valid 'nonce' in each block of transactions. If two miners in the same pool were to work on the same nonce simultaneously, it would be inefficient, and only one would receive the block reward. To prevent this, mining pools use a process called 'work assignment' or 'job distribution' to allocate work efficiently. Mining pools distribute work to individual miners or participants. When a new block needs to be mined, the pool's server (or pool operator) creates a unique 'job' for the miners. This job includes all the information needed to mine the block, such as the current list of unconfirmed transactions, the previous block's header, and the target difficulty.
        shares_heading: 'Gửi cổ phần', // Submitting Shares
        shares_paragraph:
            "Các thợ đào làm việc trên các công việc được giao này và liên tục cố gắng tìm ra nonce chính xác. Khi một thợ đào tin rằng họ có một giải pháp, họ gửi cái gọi là 'cổ phần' cho máy chủ pool. Cổ phần này chứng minh rằng thợ đào đang tích cực làm việc để giải quyết vấn đề. Cổ phần dễ tìm thấy hơn nhiều so với giải pháp thực tế, nhưng chúng đóng vai trò là bằng chứng về nỗ lực của thợ đào.",         // Miners work on these assigned jobs and continuously attempt to find the correct nonce. When a miner believes they have a solution, they submit what's called a 'share' to the pool server. This share demonstrates that the miner is actively working on solving the problem. Shares are much easier to find than the actual solution, but they serve as proof of the miner's efforts.
      },
      split: {
        title: 'Phương thức thanh toán', // Payout Schemes
        nav_title: 'Chia sẻ công bằng', // Splitting the fair share
        payout_subheading:
            'Một pool khai thác có thể chọn phân phối phần thưởng khối theo một số cách khác nhau, khác nhau ở một số cách tinh tế nhưng quan trọng:', // A mining pool can choose to distrubute the block rewards in several ways the differ in some subtle but key ways:
        pps_heading: 'Trả tiền cho mỗi cổ phần (PPS)', // Pay-Per-Share (PPS)
        pps_paragraph:
            "Trong PPS, thợ đào nhận được khoản thanh toán cố định cho mỗi cổ phần hợp lệ mà họ gửi, bất kể pool có khai thác thành công một khối hay không. Hệ thống này cung cấp cho thợ đào thu nhập ổn định và dự đoán được, làm cho nó trở thành lựa chọn ưa thích cho những người đánh giá cao tính nhất quán trong thu nhập của họ. PPS giảm thiểu sự biến động trong phần thưởng, vì thợ đào được đền bù cho công việc của họ trên cơ sở mỗi cổ phần, cung cấp một nguồn thu nhập đáng tin cậy trong thế giới khai thác tiền điện tử biến động. Tuy nhiên, PPS có thể khấu trừ một khoản phí từ thu nhập của thợ đào nếu pool không khai thác thành công một khối. Khoản phí này nhằm trang trải chi phí hoạt động và giảm thiểu rủi ro cho nhà điều hành pool trong trường hợp không khối nào được khai thác trong một khoảng thời gian nhất định. Trong PPS truyền thống, thợ đào nhận được khoản thanh toán giảm khi pool gặp khó khăn.",                 // In PPS, miners receive a fixed payout for each valid share they submit, regardless of whether the pool successfully mines a block or not. This system provides miners with a steady and predictable income, making it a preferred choice for those who value consistency in their earnings. PPS minimizes the variance in rewards, as miners are compensated for their work on a per-share basis, offering a reliable source of income in the world of volatile cryptocurrency mining. However, PPS may deduct a fee from the miner's earnings if the pool doesn't successfully mine a block. This fee is meant to cover operational costs and mitigate the risk for the pool operator in case no blocks are mined within a certain timeframe. In traditional PPS, miners receive a reduced payout when the pool faces difficulties.
        pplns_heading: 'Trả tiền cho N cổ phần cuối cùng (PPLNS)', // Pay-Per-Last-N-Shares (PPLNS)
        pplns_paragraph:
            "PPLNS tính đến đóng góp của thợ đào trong một cửa sổ cụ thể của N cổ phần cuối cùng. thợ đào được trả tiền dựa trên số lượng và độ khó của các cổ phần mà họ đã gửi trong cửa sổ đó. PPLNS khuyến khích thợ đào hoạt động tích cực trong pool, vì nó khen thưởng cho sự tham gia nhất quán và giúp giảm thiểu rủi ro của các chiến lược nhảy pool. thợ đào nhận được khoản thanh toán khi pool khai thác thành công một khối và phần thưởng được phân phối tỷ lệ thuận với những đóng góp gần đây của họ, cung cấp một cách tiếp cận công bằng và hiệu quả dựa trên hiệu suất để kiếm tiền trong hệ sinh thái khai thác.",                 // PPLNS takes into account a miner's contribution over a specific window of the last N shares. Miners are paid based on the number and difficulty of shares they've submitted within that window. PPLNS encourages miners to stay active in the pool, as it rewards consistent participation and helps to reduce the risk of pool-hopping strategies. Miners receive payouts when the pool successfully mines a block, and the rewards are distributed proportionally to their recent contributions, providing a fair and performance-driven approach to earning in the mining ecosystem.
        pps_plus_heading: 'Trả tiền cho mỗi cổ phần + (PPS+)', // Pay Per Share + (PPS+)
        pps_plus_paragraph:
            "PPS+ cung cấp cho thợ đào khoản thanh toán cố định cho mỗi cổ phần mà họ đóng góp cho nỗ lực khai thác của pool. Tuy nhiên, PPS+ thường bao gồm tiền thưởng hoặc phí bảo hiểm bổ sung như một khuyến khích cho các thợ đào đóng góp cho sự ổn định và độ tin cậy của pool. Tiền thưởng này được thiết kế để khuyến khích thợ đào hoạt động tích cực trong pool và thúc đẩy sự tham gia lâu dài. PPS+ cung cấp cho thợ đào thu nhập ổn định và dự đoán được, trong khi tiền thưởng bổ sung làm cho nó trở thành một lựa chọn hấp dẫn cho những người tìm kiếm cả độ tin cậy và phần thưởng bổ sung trong thế giới khai thác tiền điện tử.",                 // PPS+ offers miners a fixed payment for each share they contribute to the pool's mining efforts. However, PPS+ typically includes an additional bonus or premium as an incentive to miners who contribute to the pool's stability and reliability. This bonus is designed to encourage miners to stay active in the pool and foster long-term participation. PPS+ provides miners with a consistent and predictable income, while the added bonus makes it an attractive option for those seeking both reliability and extra rewards in the world of cryptocurrency mining.
        fpps_heading: 'Trả tiền đầy đủ cho mỗi cổ phần (FPPS)', // Full Pay Per Share (FPPS)
        fpps_paragraph:
            "Trong Full PPS, thợ đào nhận được khoản thanh toán cố định cho mỗi cổ phần mà họ gửi cho pool, bất kể pool có khai thác thành công một khối hay không. Phương pháp này cung cấp cho thợ đào thu nhập ổn định và dự đoán được, làm cho nó trở thành một lựa chọn hấp dẫn cho những người đánh giá cao độ tin cậy trong thu nhập của họ. Không giống như PPS truyền thống, Full PPS đảm bảo rằng thợ đào nhận được khoản thanh toán đầy đủ cho công việc đóng góp của họ, mà không có bất kỳ khoản khấu trừ nào, ngay cả khi pool gặp khó khăn. Đây là một phương pháp ổn định và đơn giản phù hợp với các thợ đào tìm kiếm thu nhập ổn định trong thế giới khai thác tiền điện tử.",         // In Full PPS, miners receive a fixed payout for every share they submit to the pool, irrespective of whether the pool successfully mines a block or not. This approach provides miners with a steady and predictable income, making it an attractive choice for those who value reliability in their earnings. Unlike traditional PPS, Full PPS ensures that miners receive their full payout for their contributed work, without any potential deductions, even if the pool faces occasional difficulties. It's a stable and straightforward method that appeals to miners seeking consistent income in the world of cryptocurrency mining.
      },
    },
  },

  chapter_four: {
    title: 'Yêu cầu 1.61 Bitcoin của bạn', // Claiming Your 1.61 Bitcoin
    paragraph_one:
        'Phù, suýt nữa thì chết mất! Bạn đang run rẩy sau cuộc chiến với BitRey, nhưng bạn cảm thấy nhẹ nhõm vì đã chiến thắng.',         // Phew, that was close! You are shaking from your battle with BitRey, but you are relieved that you won.
    paragraph_two:
        'Khi bạn ngồi xuống bàn làm việc, bạn hít một hơi thật sâu, thả mình trôi dạt và tái hiện lại những sự kiện trong ngày trong đầu.',     // As you settle at your desk, you take a deep breath, drift off, and replay the day’s events in your head.

    intro_one: {
      title: 'Giới thiệu', // Intro
      nav_title: 'Bảo vệ thành quả', // Securing the bag
      paragraph_one:
          '—HOLOCAT: “Một trong hai chúng ta nên ngủ. Bạn cần bắt đầu liên lạc với phần còn lại của các thợ đào. Họ sẽ muốn biết nhiều hơn cả thế giới.”',             // —HOLOCAT: “One of us better sleep. You need to start contacting the rest of the miners. They’ll want to know even more than the world will.”
      paragraph_two:
          'Khi bạn ngồi xuống Bàn làm việc Hover ButtLift, bạn tái hiện lại những sự kiện trong ngày. Vanderpoole. BitRey. Sự tiết lộ rằng các thợ đào không bao giờ đồng ý ngừng hoạt động để phản đối. Tất cả có thể đều đúng không? Tất cả những điều này có phải đều được dàn dựng không? Và bạn còn phải chăm sóc con mèo này trong bao lâu nữa? (Ở khoảng cách xa, một tiếng mèo kêu.)',             // As you settle at your ButtLift Hover Desk, you replay the day’s events. Vanderpoole. BitRey. The revelation that miners never agreed to shut down in protest. Could it all be true? Was this all staged? And how much longer are you supposed to look after this cat? (In the distance, something meows.)
      paragraph_three:
          'Bất cứ điều gì xảy ra tiếp theo, bạn sẽ cần tiền. Đợi đã! Bạn chưa bao giờ yêu cầu phần thưởng khai thác từ cuộc thi đấu với BitRey của mình! Bạn quyết định rút chúng.',       // Whatever happens next, you will need funds. Wait a second! You never claimed the mining rewards from your competition with BitRey! You decide to withdraw them.
    },

    public_key_one: {
      title: 'Khóa công khai', // Public key
      nav_title: 'Các cặp khóa', // Key pairs
      heading: 'Về các cặp khóa', // About key pairs
      paragraph_one:
          'Theo pool khai thác, có vẻ như bạn có 1.61 bitcoin để yêu cầu từ tất cả công việc bạn đã làm trước đó.',             // According to the mining pool, it looks like you have 1.61 bitcoin to claim from all the work you did earlier. 
      list_one: 'Khóa riêng tư', // Private key
      list_two: 'Khóa công khai', // Public key
      paragraph_two:
          "Nhưng đợi đã, bạn thậm chí còn không có ví tiền! Bạn có thể tự hỏi mình có thể mua ví ở đâu. Mặc dù bạn có thể mua phần cứng để xây dựng một số loại ví, nhưng bạn thực sự có thể chỉ cần tạo một ví bằng máy tính hoặc thiết bị di động của mình. Hãy làm điều đó!",             // But wait, you don’t even have a wallet! You might be wondering where you can buy one. While you can purchase hardware to build certain kinds of wallets, you can actually just make one with your computer or mobile device. Let’s do it!
      paragraph_three:
          'Nếu bạn đã đăng ký tài khoản tại bất kỳ thời điểm nào, bạn đã nhận được thứ gọi là mã cá nhân. Trong mật mã học, đây được gọi là “khóa riêng tư” và thường là một phần của một cặp:',             // If you signed up for an account at any point, you got something called a personal code. In cryptography, this is called a “private key” and it is often part of a pair:
      paragraph_four:
          'Chỉ cần một cặp khóa duy nhất là tất cả những gì cần thiết để tạo một ví tiền và kiểm soát các quỹ bên trong nó. Khi bạn muốn chi tiêu bitcoin, bạn sử dụng khóa riêng tư. Khi bạn muốn nhận bitcoin, bạn sử dụng khóa công khai.',       // A single key pair is all that’s needed to create a wallet and control the funds within it. When you want to spend bitcoin, you use the private key. When you want to receive bitcoin, you use the public key.
    },

    public_key_two: {
      title: 'Khóa công khai', // Public key
      nav_title: 'Mật mã học đường elip', // Elliptic curve cryptography
      paragraph_one:
          'Vì vậy, chúng ta có khóa riêng tư, đó là mã cá nhân bạn nhận được khi đăng ký. Làm thế nào để chúng ta tạo khóa công khai từ khóa riêng tư?',             // So, we have the private key, it’s the personal code you got when you signed up. How do we generate a public key from it?
      paragraph_two:
          'Để làm được điều đó, chúng ta cần xem qua một nhánh mật mã hấp dẫn được gọi là đường elip. Đây được gọi là Mật mã học đường elip, hay ECC để ngắn gọn.',             // To do that, we need to take a peek at a fascinating branch of cryptography called elliptic curves. This is called Elliptic Curve Cryptography, or ECC for short.
      paragraph_three:
          'ECC liên quan đến việc lấy các điểm nhất định trên đường elip và thực hiện phép cộng và phép nhân trên các điểm đó.',             // ECC involves taking certain points on an elliptic curve and performing addition and multiplication on the points.
      paragraph_four:
      ...
      tooltip_one: {
        highlighted: 'Điểm tạo', // Generator Point
        question: 'Điểm tạo là gì?', // What is the Generator Point?
    ...
    public_key_three: {
      title: 'Khóa công khai', // Public key
      nav_title: 'Tính toán khóa công khai', // Calculate the public key
      paragraph_one:
          'Để suy ra khóa công khai từ khóa riêng tư, chúng ta thực hiện một hoạt động đường elip lặp đi lặp lại với điểm tạo. Điểm tạo là một điểm cụ thể trên đường cong. Giá trị của nó là một phần của tiêu chuẩn secp256k1 và nó luôn giống nhau:',             // To derive a public key from a private key, we perform an elliptic curve operation repeatedly with the generator point. The generator point is a specific point on the curve. Its value is part of the secp256k1 standard and it’s always the same:
      paragraph_two:
          'Các phép toán trên đường elip tương tự như phép cộng. Do đó, việc lặp lại các phép toán đó tương tự như phép nhân. Chúng ta sử dụng ký hiệu * để mô tả thuật toán, trong đó `k` là khóa riêng tư và `P` là khóa công khai tương ứng:',       // Mathematical operations on an elliptic curve are similar to addition. Therefore, repetition of those operations is similar to multiplication. We use the * symbol to describe the algorithm, where `k` is the private key and `P` is the corresponding public key:
      python: {
        paragraph_three:
          'Hoàn thành hàm <span className="text-green">privatekey_to_publickey()</span>  chấp nhận khóa riêng tư dưới dạng chuỗi được mã hóa hex và trả về khóa công khai tương ứng dưới dạng đối tượng GE (Group Element).',         // Complete the function <span className="text-green">privatekey_to_publickey()</span>  which accepts a private key as a hex-encoded string and returns the corresponding public key as a GE (Group Element) object.
      },
      javascript: {
        paragraph_three:
          'Hoàn thành hàm <span className="text-green">privateKeyToPublicKey()</span>  chấp nhận khóa riêng tư dưới dạng chuỗi được mã hóa hex và trả về khóa công khai tương ứng dưới dạng đối tượng GE (Group Element).',         // Complete the function <span className="text-green">privateKeyToPublicKey()</span>  which accepts a private key as a hex-encoded string and returns the corresponding public key as a GE (Group Element) object.
      },
      ...
      success: 'Tuyệt vời! Khóa công khai đó khá dài. Hãy thử nén nó!'       // Good job! That public key is pretty long. Let’s try to compress it!
    },

    public_key_four: {
      title: 'Khóa công khai', // Public key
      nav_title: 'Nén khóa công khai', // Compress the public key
      paragraph_one:
          'Khóa công khai có tọa độ x và y với tổng cộng 64 byte. Điều này có thể được nén thành 33 byte bằng cách xóa tọa độ y và thêm vào trước một byte siêu dữ liệu. Byte đó sẽ chỉ ra xem tọa độ Y là chẵn hay lẻ. Vì phương trình đường elip chỉ có hai biến, nên khóa công khai đầy đủ có thể được tính toán sau đó bởi người xác thực chỉ bằng cách sử dụng x và siêu dữ liệu:',             // The public key has an x and y coordinate for a total of 64 bytes. This can be compressed into 33 bytes by removing the y coordinate and prepending a single byte of metadata. That byte will indicate if the Y coordinate is even or odd. Because the elliptic curve equation only has two variables, the complete public key can be computed later by the verifier using only x and the metadata:
      paragraph_two_javascript:
          'Byte siêu dữ liệu phải là `2` nếu y là chẵn và `3` nếu y là lẻ. Hoàn thành hàm <span className="text-green">compressPublicKey()</span> để chấp nhận khóa công khai và trả về chuỗi hex 33 byte đại diện cho khóa công khai được nén.',             // The metadata byte should be `2` if y is even and `3` if y is odd. Complete the function <span className="text-green">compressPublicKey()</span> to accept a public key and return a 33 byte hex string representing the compressed public key.
      ...
      success:
          'Xuất sắc. Bây giờ chúng ta có khóa công khai được nén của mình. Tiếp theo, chúng ta cần băm nó và mã hóa nó ở định dạng thân thiện với con người.',             // Excellent. Now we have our compressed public key. Next we need to hash it and encode it in a human-friendly format.
    },
    ...
    address_one: {
      ...
      paragraph_one:
          'Và ở đó bạn có nó! Khóa công khai được nén của bạn! Có rất nhiều điều thú vị mà chúng ta có thể làm với nó, bao gồm cả việc tạo địa chỉ cho ví của chúng tôi. Chúng ta sẽ tìm hiểu về điều đó trong thử thách tiếp theo.',             // And there you have it! Your compressed public key! There are lots of interesting things we can do with it, including generating addresses for our wallet. We’ll learn about that in the next challenge.
      paragraph_two:
          'Lưu ý rằng việc tạo khóa công khai là một chiều. Bạn không thể tìm ra khóa riêng tư được sử dụng để tạo khóa công khai trừ khi bạn giải được một bài toán toán học khét tiếng được gọi là ',             // Note that generating a public key is a one way street. You can’t figure out the private key used to generate a public key unless you solve a notoriously difficult math problem called the
      tooltip_one: {
    ...
    address_two: {
      title: 'Địa chỉ', // Address
      nav_title: 'Băm khóa công khai được nén', // Hash the compressed public key
      paragraph_one:
          'Bạn có nhớ thử thách băm không? Hóa ra bạn có thể tạo địa chỉ bitcoin đơn giản nhất bằng cách băm khóa công khai được nén của mình. Bitcoin sử dụng hai thuật toán băm khác nhau cho việc này: SHA-256 và RIPEMD-160.',       // Do you remember the hashing challenge? It turns out you can generate the simplest type of bitcoin address by hashing your compressed public key. Bitcoin uses two different hashing algorithms for this: SHA-256 and RIPEMD-160.
      paragraph_two: 'Các bước:', // Steps:
      paragraph_three:
          '<span className="indent-48">1. Thực hiện hàm băm SHA-256 trên khóa công khai được nén của bạn.</span>',       // <span className="indent-48">1. Perform a SHA-256 hash on your compressed public key.</span>
      paragraph_four:
          '<span className="indent-48">2. Thực hiện hàm băm RIPEMD-160 trên tóm tắt đầu ra SHA-256 đó. Kết quả cuối cùng sẽ là 20 byte được mã hóa dưới dạng chuỗi hex.</span>',       // <span className="indent-48">2. Perform a RIPEMD-160 hash on that SHA-256 output digest. The final result will be 20 bytes encoded as a hex string.</span>
      paragraph_five:
          'Hoàn thành một hàm chấp nhận khóa công khai được nén 33 byte dưới dạng chuỗi hex và trả về băm khóa công khai 20 byte dưới dạng chuỗi hex.',       // Complete a function that accepts a 33-byte compressed public key as a hex string and returns a 20-byte public key hash as a hex string.
      paragraph_six:
          'Đây là tài liệu cho các thư viện băm mà chúng tôi đã nhập cho bạn:\n'+ // Here is the documentation for the hashing libraries we imported for you:\n'
    ...
    address_three: {
      ...
      paragraph_one:
          'Có nhiều loại địa chỉ bitcoin khác nhau. Trong bài tập trước, chúng tôi đã tạo một băm khóa công khai được nén 20 byte. Bây giờ, chúng tôi muốn mã hóa băm đó thành địa chỉ Pay-to-Witness-Public-Key-Hash (p2wpkh) trên mạng Testnet.', // There are multiple types of bitcoin addresses. In the previous exercise, we created a 20-byte compressed public key hash. Now, we would like to encode that hash into a Pay-to-Witness-Public-Key-Hash (p2wpkh) address on the Testnet network.
      paragraph_two:
          'Đầu tiên, chúng ta cần thêm một số phiên bản chứng nhân là `0` vào băm. 21 byte kết quả này được gọi là <span className="font-bold">chương trình chứng nhân</span>.', // First we need to append a witness version number of `0` to the hash. These resulting 21 bytes are known as the <span className="font-bold">witness program</span>.
      paragraph_three:
      ...
      success: 'Bây giờ bạn đã có một địa chỉ mà bitcoin được khai thác có thể được gửi đến.', // Now you have an address that mined bitcoin can be sent to.
    },
    ...
    outro_one: {
      ...
      paragraph_two:
          'Bạn rút bitcoin từ pool khai thác vào ví mà bạn vừa tạo. Bây giờ bạn đã được tài trợ đầy đủ và sẵn sàng cho bất cứ điều gì mà Vanderpoole và BitRey ném vào bạn tiếp theo.', // You withdraw the bitcoin from the mining pool into the wallet you just created. You are now fully funded and ready for whatever Vanderpoole and BitRey throw at you next.
    },
    resources: {
      public_key_three: {
        generator_point_heading: 'Điểm tạo', // Generator point
        ...
        discrete_log_heading: 'Logarit rời rạc', // Discrete logarithm
      ...
      address_two: {
        hash_algo_heading: 'Thuật toán băm SHA-256, RIPEMD-160', // SHA-256, RIPEMD-160
        hash_algo_paragraph:
          'Các hàm băm tóm tắt bất kỳ lượng dữ liệu nào ở bất kỳ loại nào và luôn trả về kết quả có cùng kích thước. Đối với SHA256, đó là 32 byte. Đối với RIPEMD-160, đó là 20 byte. Đầu ra là xác định (luôn có cùng đầu ra cho cùng một đầu vào) nhưng nếu không, không thể phân biệt với ngẫu nhiên. Các hàm băm có hiệu quả giảm dữ liệu thành một dấu vân tay nhỏ nhất quán.', // Hash functions digest any amount of any kind of data and always return a result of the same size. For SHA256, it’s 32 bytes. For RIPEMD-160, it’s 20 bytes. The output is deterministic (always the same output for the same input) but otherwise, indistinguishable from random. Hash functions effectively reduce data to a small consistent fingerprint.
        tip_one:
          'Khi tính toán băm SHA-256, bạn sẽ cần đảm bảo băm khóa nén của mình dưới dạng byte chứ không phải hex. Nếu bạn đang sử dụng JavaScript, điều này sẽ yêu cầu chuyển đổi chuỗi hex thành bộ đệm.', // When calculating the SHA-256 hash you will need to make sure to hash your compressed key as bytes not hex. If you are using JavaScript, this will require converting the hex string to a buffer.
        tip_two:
          'Đảm bảo rằng bạn đang thực hiện các thuật toán băm theo thứ tự chính xác!', // Make sure you are doing the hashing algorithms in the correct order!
      },
      address_three: {
        wpkh_heading: 'Địa chỉ Khóa Công khai Chứng nhân (wpkh)', // Witness Public Key Hash (wpkh) address
        wpkh_paragraph:
          'Địa chỉ bitcoin là một chuỗi ký tự được thiết kế cho người dùng để xử lý. Nó ngắn gọn, dễ sao chép và dán và có một số loại kiểm tra tổng hợp tích hợp để đảm bảo rằng nó luôn được sao chép chính xác. Nó mã hóa an toàn một tập lệnh đầu ra Bitcoin mà người nhận có thể chi tiêu từ đó. Có một số loại tập lệnh đầu ra và một số cơ chế mã hóa. Trong thử thách này, chúng tôi mã hóa khóa công khai được nén bằng bech32 để tạo ra cái gọi là địa chỉ băm khóa công khai chứng nhân.', // A bitcoin address is a string of characters that is designed for users to handle. It is short, easy to copy and paste, and has some kind of built-in checksum to ensure that it is always copied correctly. It safely encodes a Bitcoin output script that the recipient can spend from. There are several types of output scripts and several encoding mechanisms. In this challenge, we encode a compressed public key with bech32 to create what is called a witness public key hash address.
  ...
  chapter_seven: {
    ...
    intro_one: {
      ...
      paragraph_three:
        'Holocat giơ móng vuốt của mình và đối phó với đội quân máy bay không người lái của Vanderpoole bằng cách nhảy vọt. Mika 3000 đưa cho bạn một chiếc áo len cổ lọ màu đen, găng tay và một cặp kính nhìn đêm.', // Holocat extends her claws and deals with Vanderpoole’s drone army pounce by pounce. Mika 3000 hands you a black turtleneck sweater, gloves, and a pair of night vision goggles.
      paragraph_four:
      ...
      paragraph_five:
        'Tự hỏi mình đã dính vào chuyện gì, bạn và Mika 3000 leo lên tường thành của dinh thự chính của Vanderpoole. Đến ngạc nhiên, ngôi nhà của anh ta tràn ngập nghệ thuật, sách và tạp chí giấy ủng hộ Bitcoin. Anh ta là, hoặc đúng hơn là đã từng là, một người hâm mộ Bitcoin thực thụ. Một bộ sưu tập các máy đào mỏ có niên đại 125 năm tuổi được bảo quản trong tủ kính trưng bày. Đó là lúc bạn nhìn thấy nó: xưởng làm việc của Vanderpoole.', // Wondering what you’ve gotten yourself into, you and Mika 3000 scale the castle walls of Vanderpoole’s primary residence. To your surprise, his home overflows with pro-bitcoin art, books, and paper magazines. He is, or rather was, a true bitcoiner. A collection of miners dating back 125 years sit preserved in glass display cases. That’s when you see it: Vanderpoole’s workshop.
    },
    intro_two: {
    ...
    intro_three: {
      ...
      paragraph_one:
        "Nhưng bạn không thể chỉ để mã khai thác bị tàn tật trên máy chủ của Vanderpoole. Nó sẽ tiếp tục lây nhiễm cho các thợ đào. Bạn phải sửa chữa nó! Khi bạn đọc qua mã, Holocat cho bạn xem màn hình hiển thị mempool đang tràn ngập các giao dịch chưa được xác nhận. Càng sớm sửa chữa mã này thì càng tốt.", // But you can't just leave the crippled mining code on Vanderpoole's server. It will keep infecting miners. You have to fix it! As you read through the code, Holocat shows you a mempool display that is filling with unconfirmed transactions. The sooner this code gets fixed, the better.
    },
    mempool_transaction_one: {
      ...
      paragraph_two: {
        a: 'Bạn cần sửa chữa hàm lắp ráp khối không chỉ để xây dựng các khối hợp lệ mà còn tối đa hóa phí trong khối, để các thợ đào có thể kiếm được nhiều lợi nhuận nhất có thể theo các quy tắc đồng thuận. Các quy tắc đồng thuận quan trọng nhất mà bạn cần chú ý để hạn chế tổng ', // You need to fix the block assembly function not only to build valid blocks but also maximize the fees in the block, so the miners can earn the most profit possible given the consensus rules. The most critical consensus rules you will need to pay attention to restrict the total
        b: {
  ...
  chapter_eight: {
    ...
    building_blocks_four: {
      ...
      paragraph_one: `Mỗi nút đầy đủ Bitcoin đều có một cơ sở dữ liệu. Đó là nơi lưu trữ và lập chỉ mục các khối theo băm của chúng. Nút đầy đủ theo dõi các khối nào là ứng viên ở mỗi độ cao trong chuỗi với một chỉ mục thứ hai ánh xạ chiều cao -> [băm khối].`, // Each bitcoin full node has a database. That's where blocks are stored and indexed by their hash. The full node keeps track of which blocks are candidates at each height in the chain with a second index that maps height -> [block hashes].
      ...
      paragraph_three: `Truy xuất tất cả các ứng viên khối ở độ cao 6929996 và in băm của khối có ít giao dịch nhất trong số đó.`, // Retrieve all the block candidates at height 6929996 and print the hash of the block with the fewest transactions in it.
      success: `Làm tốt`, // Nicely Done
    },
    building_blocks_five: {
      ...
      paragraph_five: `trong một khối có băm:`, // in a block with the hash:
    ...
    building_blocks_six: {
      ...
      list_one:
        'Bắt đầu với khối #1 được khai thác vào năm 2009, trợ cấp khối là 50 BTC (hoặc 5.000.000.000 satoshi)', // Starting with the block #1 mined in 2009, the block subsidy is 50 BTC (or 5,000,000,000 satoshis)
    ...
    building_blocks_eight: {
      ...
      paragraph_four: `Hãy nhớ rằng: Các đối tượng khối được trả về bởi API JSON có một thuộc tính "prev" xác định cha mẹ của khối đó bằng băm của nó:`, // Remember: Block objects returned by the JSON API have a property "prev" which identifies that block's parent by its hash:
    ...
    outro_one: {
      ...
      paragraph_one:
        "Bạn đã tìm thấy chuỗi dài nhất và chứng minh điều đó cho mọi người thấy! Danh sách các khối không hợp lệ mà bạn tìm thấy rất ấn tượng. Vanderpoole đã rất nỗ lực để khiến mọi người chấp nhận các khối của anh ta chứa trợ cấp, nhưng anh ta không thể làm gì để thay đổi các quy tắc của Bitcoin. Như chúng ta đã quan sát thấy, chỉ vì một khối là một phần của một chuỗi, điều đó không nhất thiết có nghĩa là nó hợp lệ. Bây giờ, bạn đã tiến gần hơn một bước để bôi nhọ danh dự của Vanderpoole. Không cần phải nói, anh ta đã không trả lời phần còn lại của câu hỏi của bà Chunk.", // You found the longest chain and proved it to everyone! The list of invalid blocks you found is impressively long. Vanderpoole went to great lengths to try to get people to accept his blocks containing subsidies, but he couldn't do anything to change the rules of bitcoin. As we observed, just because a block is part of a chain, it does not necessarily mean that it is valid. Now, you are one step closer to discrediting Vanderpoole. Needless to say, he didn't answer the remainder of Ms. Chunk's questions.
    },
    resources: {
      building_blocks_three: {
  ...
  chapter_nine: {
    ...
    intro_one: {
      ...
      paragraph_one:
        'Mọi người đang ăn mừng; tất cả mọi người đều đang cười. Nhưng có một người đàn ông trên trường quay không vui: một người đàn ông cao lớn, ăn mặc lịch sự, tóc hoa râm mà bạn nhận ra ngay lập tức. Đám đông tách ra và bạn nhìn thấy anh ta mặt đối mặt lần đầu tiên: Vanderpoole!', // Everyone is celebrating; everyone is smiling. But there is one man on set who isn’t happy: a tall, well-dressed, gray-haired man who you recognize immediately. The crowd parts, and you see him face to face for the first time: Vanderpoole!
      paragraph_two:
        'Hai tay anh ta nắm chặt. Bạn đã phá hủy công việc kinh doanh của anh ta và vạch trần trò lừa đảo lâu đời của gia đình anh ta. Điều đầu tiên anh ta thốt ra là lời đe dọa sẽ kiện bạn mỗi ngày trong suốt quãng đời còn lại của bạn "nhân vô cực".', // His fists are clenched. You have ruined his business and exposed his family’s long con. The first thing out of his mouth is a threat to sue you every day for the rest of your life "times infinity".
    },
    intro_two: {
      title: 'Giới thiệu', // Intro
    ...
    opcodes_three: {
      ...
      paragraph_five:
        'Script này sẽ được băm và được mã hóa bech32 thành một địa chỉ mà ai đó có thể gửi tiền đến.', // This script will be hashed and bech32-encoded into an address where someone can send coins.
    },
    opcodes_four: {
      title: 'OpCodes', // OpCodes
    ...
    opcodes_six: {
      ...
      paragraph_one:
        'Chúng tôi đã khám phá "thanh toán cho băm khóa công khai" trong các chương trước. Đây là script Bitcoin được viết rõ ràng trong hàng triệu đầu ra giao dịch trước khi chứng kiến ​​tách biệt xuất hiện và rút gọn nó. Đồng xu bị khóa bởi băm của một khóa công khai. Người chi tiêu phải tiết lộ khóa công khai khớp với băm đó và sau đó cung cấp chữ ký được xác minh bởi khóa công khai đó.', // We've explored "pay to public key hash" in previous chapters. This is the bitcoin script that was written explicitly in millions of transaction outputs before segregated witness came along and abbreviated it. Coins are locked by the hash of a public key. The spender must reveal the public key that matches that hash, and then provide a signature verified by that public key.
    ...
    opcodes_seven: {
      ...
      paragraph_one:
        'Chính sách chữ ký đa phương cung cấp một danh sách các khóa công khai và số lượng chữ ký bắt buộc đối với một chi tiêu hợp lệ. Nó có thể được mô tả là "m-trong-n" có nghĩa là "m số lượng chữ ký được yêu cầu từ danh sách n khóa công khai này". Các khóa công khai và các giá trị m và n thường được bao gồm trong script khóa và người chi tiêu chỉ cần cung cấp đúng số lượng chữ ký.', // Multisignature policies provide a list of public keys and a number of signatures required for a valid spend. It can be described as "m-of-n" meaning "m number of signatures are required from this list of n public keys". The public keys and the m and n values are typically included in the locking script and the spender only needs to provide the right number of signatures.
      paragraph_two:
        'Holocat xuất hiện với một tin nhắn được ghi âm trước từ Satoshi Nakamoto!', // Holocat appears with a pre-recorded message from Satoshi Nakamoto!
      paragraph_three:
    ...
    opcodes_eight: {
      ...
      multisig_list_two:
        'Bỏ <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">n</span> số mục khỏi ngăn xếp, tất cả đều được mong đợi là các khóa công khai ở định dạng <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">PUBKEY(...)</span>', // Pop <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">n</span> number of items off the stack, these are all expected to be public keys of the format <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">PUBKEY(...)</span>
      multisig_list_three:
      ...
      multisig_list_six:
        'Lặp lại từng khóa công khai: Xác minh khóa với chữ ký trên cùng của ngăn xếp. Nếu hợp lệ, hãy xóa cả khóa và chữ ký và tiếp tục với khóa công khai tiếp theo. Nếu không hợp lệ, chỉ xóa khóa công khai và tiếp tục với khóa công khai tiếp theo (sẽ bắt đầu bằng cách kiểm tra với chữ ký trên cùng đó)', // Iterate through each public key: Verify the key against the stack-topmost signature. If it is valid, remove both the key and the signature and continue with the next public key. If it is not valid, remove the public key only and continue to the next public key (which will begin by checking against that same topmost signature)
      multisig_list_seven:
        'Nếu tất cả các khóa công khai đã được kiểm tra và vẫn còn chữ ký nào đó, thao tác sẽ thất bại.', // If all public keys have been tested and there are any signatures remaining, the operation fails.
      multisig_list_eight:
        'Sau khi tất cả các chữ ký đã được xóa, thao tác có thể kết thúc sớm với thành công, ngay cả khi vẫn còn nhiều khóa công khai.', // Once all signatures have been removed the operation can finish early with success, even if more public keys remain.
      paragraph_one:
        'Lưu ý rằng m <= n. Có thể có nhiều khóa công khai hơn chữ ký nhưng không bao giờ có nhiều chữ ký hơn khóa công khai. Ngoài ra, hãy lưu ý rằng các khóa và chữ ký PHẢI theo cùng một thứ tự, ngay cả khi một số khóa không được sử dụng để ký.', // Note that m <= n. There may be more public keys than signatures but never more signatures than public keys. Also note that the keys and signatures MUST be in the same order, even if some keys are not used to sign.
      paragraph_two: 'Cung cấp ngăn xếp ban đầu để chi tiêu từ script.', // Provide the initial stack to spend from the script.
    },
    opcodes_nine: {
      title: 'OpCodes', // OpCodes
    ...
    proposal_three: {
      ...
      nav_title: 'Làm cho anh ta chờ đợi', // Make him wait
      heading: 'Giao dịch bị khóa thời gian có điều kiện', // Conditional time locked transaction
      paragraph_one: `Chờ một chút, điều đó không có ý nghĩa gì - bạn không muốn phải đối phó với anh ta mãi mãi! Thỏa thuận mới là bạn nhận được tất cả các khoản quyên góp trong hai giờ tiếp theo trong khi bạn vẫn đang ở trên TV. Quỹ Lil Bits nhận được bất cứ điều gì đến sau đó. Bạn nhìn vào khối Bitcoin trên tường trong studio và đồng ý rằng chiều cao khối 6930300 có thể sẽ được khai thác trong khoảng hai giờ.`, // Wait a minute, that doesn't make sense—you don't want to deal with him forever! The new deal is you get all donations for the next two hours while you are still on TV. The Lil Bits Foundation gets anything that comes in afterwards. You look at the bitcoin block block on the wall in the studio and agree that block height 6930300 will probably be mined in about two hours.
      paragraph_two: `Hãy nhớ rằng khóa công khai của Vanderpoole là PUBKEY(vanderpoole) và của bạn là PUBKEY(me).`, // Remember Vanderpoole's public key is PUBKEY(vanderpoole) and yours is PUBKEY(me).
    ...
    resources: {
      ...
      opcodes_six: {
        ...
        spoiler: `Script kiểm tra chữ ký so với khóa công khai đã băm. Đặt một khóa chữ ký trước, sau đó là một khóa công khai` // The script checks a signature against a hashed public key. Put a signature key first then a public key
      },
      opcodes_eight: {
      ...
      proposal_two: {
        ...
        spoiler: `Gợi ý Script: Thỏa thuận đa chữ ký này yêu cầu cả hai bên tham gia ký. Script chỉ định rằng hai chữ ký là bắt buộc và bao gồm cả hai khóa công khai. 
...
Gợi ý ngăn xếp: Để chi tiêu trước khi bí mật được tiết lộ, Vanderpoole sử dụng chữ ký của mình. Sau khi bí mật được tiết lộ, bạn sử dụng chữ ký của mình, băm của bí mật và cung cấp 0 vì script đã vượt qua xác minh chưa tiết lộ.`,
...
//Stack Hint: To spend before the secret is revealed, Vanderpoole uses his signature. After the secret is revealed, you use your signature, a hash of the secret, and provide a 0 because the script has moved past the unrevealed verification.
  ...
  chapter_ten: {
    ...
    intro_two: {
      ...
      paragraph_three:
        '—MIKA 3000: “Uh oh. Chiêu trò nhỏ của bạn ở phía sau đã gây ra một số vấn đề về phí khai thác. Mạng lưới bitcoin đang trải qua sự gia tăng hoạt động và phí giao dịch đang tăng vọt! Chúng ta sẽ phải trả tiền cho điều này ngoại tuyến.”', //   '—MIKA 3000: “Uh oh. Your little stunt back there did a number on miner fees. The bitcoin network is experiencing a surge in activity and transaction fees are through the roof! We’re going to have to pay for this off-chain.”'
    },
    intro_three: {
      title: 'Ngoại tuyến?', // title: 'Off-chain?'
      nav_title: 'Ngoại tuyến?', // nav_title: 'Off-chain?'
      paragraph_one: `Kể từ khi “Satoshi” lần đầu tiên tiếp cận, bạn chỉ phải làm việc với các giao dịch trên chuỗi. Đó là cách bạn yêu cầu phần thưởng khai thác và cách bạn chuyển tiền cho Mika 3000. Tuy nhiên, các giao dịch trên chuỗi này không lý tưởng cho việc sử dụng hàng ngày thường xuyên vì không gian khối bị hạn chế và phí khai thác có thể thay đổi.`, // paragraph_one: `Since “Satoshi” initially reached out, you’ve only had to work with on-chain transactions. That’s how you claimed your mining rewards and how you transferred funds to Mika 3000. However, these on-chain transactions aren’t ideal for frequent, everyday use because block space is limited and miner fees can vary.`
    ...
    opening_a_channel_two: {
      ...
      off_chain_list_two:
          "Để lấy tiền, Laszlo sẽ cần phát sóng giao dịch này lên mạng và điều đó yêu cầu một khoản phí khai thác. Hãy dành riêng 1000 sats để trả phí đó. Cập nhật số tiền trong đầu ra tiền thừa của bạn để tính đến điều này.", // "To get the money, Laszlo will need to broadcast this transaction to the network, and that requires a miner fee. Let's set aside 1000 sats to pay that fee. Update the amount in your change output to account for this."
    ...
    opening_a_channel_four: {
      ...
      hint_two: `Đừng quên dành riêng phí khai thác`, // hint_two: `Don't forget to set aside miner fees`
    },
    ...
    making_a_payment_one: {
      ...
      list_three_sub_two:
          'Laszlo nhận được tất cả tiền nếu anh ta lấy được khóa riêng tư <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base">REVOCATION_YOU_1</span> từ bạn', //  'Laszlo gets all the money if he gets the private key <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base">REVOCATION_YOU_1</span> from you'
      list_four:
    ...
    making_a_payment_four: {
      ...
      paragraph_one: `Đó là lúc Holocat xuất hiện trên bàn, đứng thẳng bằng hai chân sau với hai chân trước dang rộng và kêu meo meo.`, // paragraph_one: `That's when Holocat materializes on the table, standing up on her hind legs with her front paws outstretched, and meows.`
    ...
    making_a_payment_five: {
      ...
      paragraph_one: `Chúng ta biết rằng chúng ta muốn Laszlo ký giao dịch đầu tiên nhưng chúng ta không muốn anh ta có chữ ký của chúng ta trên đó. Chúng ta sẽ cần phải xây dựng một giao dịch thứ hai cho anh ta có <span className="font-bold">chứa</span> chữ ký của chúng ta, nhưng không có khả năng anh ta chi tiêu tất cả tiền một cách bất công.`, // paragraph_one: `We know we want Laszlo to sign the first transaction but we don't want him to have our signature on it. We'll need to construct a second transaction for him that <span className="font-bold">does</span> have our signature on it, but without any possibility of him spending all the money unfairly.`
    ...
    making_a_payment_six: {
      ...
      commitment_one_you: {
        ...
        list_three: `<span className="font-semibold">Phí khai thác:</span> 1.000 sats`, // list_three: `<span className="font-semibold">Miner fees:</span> 1,000 sats`
      },
      commitment_two_you: {
        list_one:
            '<span className="font-semibold">Đầu vào 0:</span> được ký bởi Laszlo', // '<span className="font-semibold">Input 0:</span> signed by Laszlo'
        ...
        list_four: `<span className="font-semibold">Phí khai thác:</span> 1.000 sats`, // list_four: `<span className="font-semibold">Miner fees:</span> 1,000 sats`
      },
    ...
    making_a_payment_eight: {
      ...
      heading_three: 'Gợi ý', // heading_three: 'Hints'
      paragraph_four: `Đừng quên dành riêng 1.000 sats cho phí khai thác`, // paragraph_four: `Don't forget to set aside 1,000 sats for miner fees`
    ...
    sats_distribution:
        'Laszlo nên nhận đủ số sats cho bia và không chịu trách nhiệm về phí khai thác 1.000 sat. Phí được trừ vào tiền thừa của bạn.', // sats_distribution: 'Laszlo should receive enough sats for the beer, and is not responsible for the 1,000 sat miner fees. The fee comes out of your change.'
    output_script:
    ...
    miner_fees_basic: `Đừng quên dành riêng 1.000 sats cho phí khai thác.`, // miner_fees_basic: `Don't forget to set aside 1,000 sats for miner fees.`
    miner_fees: `Đừng quên dành riêng 1.000 sats từ tiền thừa của bạn cho phí khai thác.`, // miner_fees: `Don't forget to set aside 1,000 sats from your change for miner fees.`
    },
  },
  ...
  modal_signin: {
    ...
    pre_signin_paragraph_one:
        'Nhập khóa riêng tư của bạn bên dưới để khôi phục tài khoản và tiến trình của bạn.', // pre_signin_paragraph_one: 'Enter your private key below to restore your account and progress.'
    ...
    prompt: 'Nhập khóa riêng tư của bạn', // prompt: 'Enter your private key'
  ...
  modal_signup: {
    ...
    subheading_two: 'Sao lưu khóa riêng tư của bạn', // subheading_two: 'Back up your private key'
    generate:
    ...
    acknowledged:
        'Tôi xác nhận rằng tôi đã lưu khóa riêng tư này và biết rằng tôi sẽ cần nó cho các thử thách trong tương lai.', // acknowledged:'I acknowledge I have saved this private key and know that I will need it for challenges in the future.'
  },
  ...
  hasher: {
    ...
    return_hash: 'Dưới đây bạn sẽ thấy đầu vào của bạn được chuyển đổi thành một hàm băm', // return_hash: 'Below you will see your input converted to a hash'
  },
  ...
  error: {
    ...
    second: 'Holocat có thể đã gặm nhấm một số dây cáp một lần nữa. Con mèo xấu xa!', // Holocat may have chewed on some cables again. Bad cat!
