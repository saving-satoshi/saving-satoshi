const translations = {
  ...
  chapter_one: {
    title: `눈앞에 숨겨진 비밀`,
    paragraph_one: `2139년. 마지막 비트코인이 채굴되기까지 단 2주 남았습니다. 사토시 광장에선 카운트다운 시계가 수개월째 돌아가고 있습니다. 지금까지 모든 블록에는 비트코인 보상이 있었지만, 곧 그 보상도 끝납니다. 100년 넘게 이어져 온 채굴 끝에 마침내 마지막 비트코인 채굴이 눈앞에 다가왔습니다. 전세계가 마지막 블록이 채굴되는 역사적인 사건이자, 한 시대가 끝나는 순간일지도 모릅니다.`,
    ...
    intro_one: {
      ...
      paragraph_three: `—반더풀: "네, 데보라. 처음부터 설명할게요. 우리 반더풀 집안, 아버지와 할아버지 때부터 블록 21,000 때부터 채굴을 해왔죠. 그래서 단언할 수 있어요. 지금 이 지연은, 전 세계 채굴자들이 의도적으로 채굴기를 껐기 때문입니다. 이건 시위입니다. 아무도 비트코인 발행이 2,100만 개에서 멈추는 걸 원치 않아요. 채굴자들은 수수료만으론 살 수 없거든요.”`,
      start: `계속하기`,
    },
    ...
    intro_two: {
      ...
      paragraph_four: `정말일까?`,
      paragraph_five: `홀로캣: "얼른 시작해야 해. 내가 도와줄 수 있어. 하지만 서둘러야 해. 야옹~”`,
    },
    ...
    transacting_one: {
      title: `트랜잭션`,
      nav_title: `트랜잭션에는 무엇이 있을까?`,
      heading: `트랜잭션에는 뭐가 들어있을까?`,
      paragraph_one: `트랜잭션에서 가장 중요한 요소는 '입력'과 '출력'입니다. 이전 과제에서는 입력에 숨겨진 비밀 메시지를 해독했죠. 이번에는 출력에 담긴 메시지를 해독해봅시다.`,
    ...
    transacting_two: {
      ...
      paragraph_four: `3. 이제 SCRIPTPUBKEY(ASM) 항목을 찾으세요. OP_RETURN OP_PUSHBYTES_33 부분이 보일 거예요. 이것들은 오퍼코드(opcode)라고 부르며, 우리가 주목할 부분은 그 이후에 나오는 데이터입니다.`,
    ...
    outro_one: {
      ...
      paragraph_two: `당신은 심한 고양이 알레르기가 있어서 홀로그램조차 버거울 지경이지만, 홀로캣은 따라옵니다. 따라오지 말라고 해도… 고양이는 원래 말을 안 듣잖아요?`,
      button_text: `챕터1 완료`,
    },
    ...
    resources: {
      genesis_two: {
        ...
        scriptsig_paragraph: `'스크립트 시그는 트랜잭션의 입력 부분에 포함되는 스크립트로, 해당 비트코인을 사용할 수 있는 권한이 자신에게 있음을 증명하는 역할을 해요. 일종의 잠금 해제 키처럼 작동하며, 참조된 UTXO를 사용할 수 있다는 증거를 제공합니다.`,
      ...
      transacting_two: {
        ...
        bitcoin_script_paragraph: `비트코인 스크립트(Bitcoin Script)는 비트코인 거래에서 사용되는 간단한 스택 기반 프로그래밍 언어입니다. 다양한 오퍼코드(opcode)를 통해 스크립트 내부 데이터에 어떤 연산을 수행할지 정의합니다.`,
  ...
  chapter_two: {
    title: `해시 계획 수립`,
    paragraph_one: `Satoshi가 알려준 좌표에 도착했어요. 예상대로, 그곳은 낡고 으스스한 창고였죠.`,
    paragraph_two: ` 당신은 아버지의 낡은 버짓콥터를 타고 그 창고 주위를 세바퀴나 돌며 망설입니다. 당신은 깊게 한숨을 쉬고, 결국 안으로 들어가기로 결심합니다. 버짓콥터의 열 감지기로 확인해봐도, 어둠 외에는 아무것도 감지되지 않습니다. 이 장소는 오랫동안 아무도 찾아오지 않은걸로 보입니다.`,
    ...
    intro_one: {
      ...
      paragraph_three: {
        ...
        b: ` 그 칩이 하는일은 하나 뿐이야. 바로 비트코인 채굴이지. 오직 한 가지 일만 하다 보니, 효율도 높고 에너지도 적게 들지. 덕분에 채굴 경쟁력도 생겼고. 믿기진 않겠지만 예전엔 노트북으로도 채굴을 했었으니까."`,
      },
      paragraph_four: `이제 반더풀 가문이 남긴 이 오래된 채굴기들의 정체가 확실해졌네요.`,
      tooltip_one: {
        question: `ASIC 채굴기란?`,
    ...
    scripting_one: {
      ...
      paragraph_two: `채굴자들은 블록에 넣고 싶은 모든 정보를 모읍니다. 예를 들어 이전 블록 헤더 해시, 블록에 포함할 거래들의 해시(코인베이스 거래 포함), 시간 정보 등을 모아 무작위 숫자인 논스(Nonce)와 결합합니다. 논스는 말 그대로 한 번만 사용하는 숫자예요. 이 모든 정보를 해시 함수에 넣어서 "블록 해시"를 생성합니다.`,
    ...
    scripting_two: {
      title: `자동화`,
      nav_title: `해시함수 만들기`,
      paragraph_one: `자 이제 직접 코드를 작성하고 실행해봅시다. 앞에 0이 다섯 개로 시작하는 SHA-256 해시를 생성하는 스크립트를 작성해보세요. sha256 함수를 반복해서 호출하며 입력값을 바꿔가며 원하는 해시를 찾아보세요. 반복문에서 숫자를 하나씩 늘려가며 입력값을 바꿔보세요. 이런 숫자를 암호학에선 '논스(Nonce)'라고 부르며, 한 번만 사용하는 특별한 값이에요.`,
      python: {
    ...
    mining_one: {
      ...
      paragraph_ten: `저 해시를 계산하는데 정말 많은 컴퓨터 연산 처리 능력이 필요했어요. 논스를 얼마나 많이 시도했는지 한번 보세요. 해시 파워가 커진 게 도움이 됐습니다. ‘해시파워’는 초당 얼마나 많은 해시를 계산할 수 있는지를 의미해요.`,
      paragraph_eleven: `각 블록에 3,500개 트랜잭션이 들어있으니 채굴된 100개 블록에는 35만 건의 트랜잭션이 확인됐다고 볼 수 있습니다.`,
      paragraph_twelve: `채굴한 덕분에 비트코인 보상을 받았어요. 비트코인 네트워크가 각 블록마다 지급하는 비트코인 블록 보상뿐 아니라 거래 수수료도 함께 받았어요.`,
  ...
  chapter_three: {
    ...
    intro_one: {
      title: `들어가며`,
      nav_title: `계획 세우기`,
      paragraph_one: `—사토시 나카모토: “야! 맞아, 너 말이야! 지금 비트코인이 51% 공격을 당하고 있어. 너가 채굴장비를 켜고난 뒤 반더풀이 다시 비트레이(BitRey)의 ASIC을 켜서 빈 블록을 채굴하고 있어. 그 장비만 문제가 아니야. 반더풀은 표준 ASIC 펌웨어에 숨겨진 백도어를 이용해서 기존 채굴기에 바이러스를 심었어. 이 바이러스 때문에 채굴기에 빈블록만 채굴되고 있어. 반더풀이 비트코인 생태계를 인질로 삼아서 비트코인 발행량을 늘리자고 협박하는 중이야. 뭐라도 빨리해봐!”`,
      ...
      paragraph_three: `—홀로캣: “우리 아직 할 일이 더 남았어! 아, 우리가 아니라 '너'가 할 일이지만. 나는 벽이나 뚫고 쥐나 놀래켜야겠다~”`,
    },

    solo_one: {
      title: `당신 vs BitRey`,
      nav_title: `혼자 채굴하기`,
      step_zero_heading: `한번 시도해봅시다`,
    ...
    coop_two: {
      ...
      paragraph_one: `Stratum 채굴 풀 프로토콜(비트코인 자체 프로토콜 외 프로토콜)에서는 코인베이스 트랜잭션에도 'extraNonce'라는 값이 포함돼 있습니다. 채굴 풀은 채굴자들에게 블록 데이터를 제공할 때 extra Nonce를 "extranonce1", "extranonce2" 처럼 두 부분으로 나눠 전달합니다.`,
    ...
    split_one: {
      ...
      paragraph_three: `이 보상은 두 가지로 구성돼 있습니다:`,
      list_one: `블록 안에 포함된 모든 거래에서 발생한 수수료`,
      list_two: `블록 보조금인, 새로 생성된 비트코인`,
      paragraph_four: `채굴자들과 함께 총 7.41 BTC를 받았어요. 이제 이걸 어떻게 나눌지 고민 중이에요. 각자의 기여도에 따라 공정하게 분배하는 게 좋겠죠.`,
      paragraph_five: `채굴풀은 각 채굴자가 찾아낸 '부분 해시 결과(Partial Solution)'의 수를 바탕으로 보상을 나눠요. 풀은 해시파워에 따라 각 채굴자에게 블록 난이도보다 낮은 최소 난이도를 부여합니다. 채굴자는 몇 초마다 이 낮은 난이도의 해시를 찾아내고 풀에 보고하고, 풀은 제출된 모든 쉐어를 기준으로 보상을 나눠줍니다.`,
    ...
    split_two: {
      ...
      step_zero_paragraph: `다시 시뮬레이션을 시행해봅시다. 채굴풀 안에서 각 채굴자가 찾은 부분 해시 결과의 개수에만 집중해보세요.`,
      ...
      step_four_paragraph: `블록을 발견할 확률은 매우 낮고, 어느 정도 운이 따라줘야 합니다. 특히 해시파워가 아주 강력한 채굴자들이 있다면 작은 채굴자들이 열심히 해도 블록을 하나도 못 찾을 수 있습니다.`,
      step_five_heading: `부분 해시 결과(Partial solution) 비율`,
      ...
      step_six_paragraph: `이제 7.41개의 비트코인 보상을 각 채굴자가 제출한 부분 해시 결과의 제출 비율에 따라 공평하게 나눌 수 있어요. 축하합니다!`,
    },
  ...
  chapter_four: {
    ...
    intro_one: {
      ...
      paragraph_two: `당신은 ButtLift Hover 책상에 앉아 오늘 있었던 일들을 머릿속으로 뒤집어 봅니다. 반더풀, BitRey, 채굴자들이 항의로 셧다운에 동의한 적 없다는 충격적인 사실을 떠올립니다. 이게 정말 다 시실일지, 누군가 다 계획한 연극은 아니었을지 혼자 생각합니다. 그리고 이 고양이는 언제까지 돌봐야 하는 건지 고민합니다. (멀리서 야옹 소리가 들립니다.)`,
    ...
    address_three: {
      ...
      paragraph_one: `비트코인 주소에는 여러가지 종류가 있어요. 이전 단계에서는 20바이트짜리 압축된 공개키 해시를 만들었습니다. 이번에는 그 해시를 Pay-to-Witness-Public-Key-Hash(P2WPKH) 형식으로 바꿔서 테스트넷 주소를 생성해보겠습니다.`,
      ...
      paragraph_five: `우리는 테스트넷 주소를 만드는 중이니 접두어 'tb'를 사용할 거예요.`,
  ...
  chapter_five: {
    ...
    paragraph_one: `밤이 늦었습니다. 피곤에 지쳐 눈을 감으려는 찰나, 두 가지 나쁜 소식이 한꺼번에 날아듭니다.`,
    ...
    intro_one: {
      ...
      paragraph_three: `—데보라 청크: “그렇다면 사토시 나카모토의 개인키를 소유하고 계신다는 걸 증명해 주실 수 있나요?”`,
    ...
    intro_two: {
      ...
      paragraph_one: `—반더풀: "용기가 없었습니다. 우리 집안의 내력에 대해선 늘 말이 많았으니까요. 제가 나서기만 했어도 쉽게 밝혀질 이야기였지만... 저는 사람들의 큰 관심이 부담스러웠습니다. 그저 소박하고 운둔하는 삶을 살고 싶었거든요. 제 개인 전용섬 중에서도 가장 마음에 드는 섬에 있는 14세기에 지어진 성에서, 카우보이 농장과 자쿠지를 오가며 매주 이런 인터뷰를 다섯 여섯 개쯤 조용히 소화하는 정도로요.”`,
    ...
    derive_message_one: {
      ...
      code_one: `-----비트코인 서명된 메시지 시작----- \n \n 나는 반더풀이고, 사토시가 블록 #170에서 처음 서명한 비트코인 거래에 사용한 개인키를 갖고 있다. 이 메시지는 그 개인키로 서명했다. \n \n -----비트코인 서명 시작----- \n \n`,
    ...
    derive_message_two: {
      nav_title: `공개키를 찾아라`,
      paragraph_one: `챕터 4에서 배웠듯이 개인키는 엄청나게 긴 무작위 숫자이며, 만든 사람만 알고 비밀로 보관해야 합니다. 우리는타원 곡선 수학을 이용해 이 개인키로 공개키를 계산할 수 있어요.`,
    ...
    derive_message_four: {
      ...
      paragraph_one: `사토시가 블록 #9를 채굴하면서 받은 보상 비트코인에는 공개키가 포함돼 있어요.`,
    ...
    verify_signature_three: {
      ...
      paragraph_two: `우리가 추출해야 할 값은 32바이트 길이의 숫자 두 개입니다. 하나는 R, 다른 하나는 S라고 부릅니다. 이 값들은 DER 구조 안에서 각각 "0220"이라는 바이트로 시작됩니다. DER 형식 전체를 복잡하게 분석할 필요는 없습니다. 그냥 0200으로 시작하는 부분을 찾아서 R 값과 S 값만 따로 복사해서 붙여놓으면 됩니다.`,
      paragraph_three: `# 아래는 할 피니에게 보낸 트랜잭션(블록 #170)에서 scriptSig에 들어있던 사토시의 서명 값입니다.`,
    },
    verify_signature_four: {
      ...
      label_one: `x 좌표 붙여넣기`,
      label_two: `y 좌표 붙여넣기`,
      paragraph_one: `4장에서 배웠듯이, 공개키는 실제로 ECDSA 타원곡선 위에 있는 하나의 점입니다. 그래서 항상 x좌표와 y좌표 두 값을 가지고 있습니다. 공개키의 첫 바이트가 04로 시작되면, 그건 압축되지 않은 형식(uncompressed) 입니다. (4장에서 배운 02, 03은 압축된 형태였습니다.) 이 04는 떼어내고, 나머지 64바이트 데이터를 32바이트씩 나누면 앞의 32 바이트는 x좌표, 뒤의 32바이트는 y좌표가 됩니다. 이제 그 두 값을 복사해서 붙여넣으면 됩니다.`,
    ...
    verify_signature_five: {
      ...
      paragraph_one: `이제 ECDSA 계산을 직접 수행할 준비를 마쳤습니다.`,
    ...
    validate_signature_two: {
      ...
      paragraph_one: `반더풀이 사용한 비트코인 메시지 서명 프로토콜에서는 서명을 base64 형식으로 인코딩하도록 되어 있습니다. 우리는 이 base64 문자열을 총 65바이트 길이의 바이트 시퀀스로 디코딩해야 합니다. 맨 앞의 1바이트 메타데이터는 제외한 후, 그 다음 남은 64바이트 데이터는 32바이트짜리 r값과 s값으로 구성돼 있습니다. (6단계에서 봤던 바로 그 r, s값 입니다!)`,
      javascript: {
        paragraph_two: {
    ...
    validate_signature_three: {
      title: `서명이 진짜인지 검증하기`,
      nav_title: `반더풀이 거짓말 한 건지 한번 확인해봅시다.`,
      heading: `그래서 반더풀은 정말 거짓말쟁이일까요?`,
      paragraph_one: `이제 프로그램에 필요한 요소들을 모두 모아, 반더풀의 서명이 사토시 공개키에 연결된 개인키로부터 나온 것인지 검증해봅시다. 제공된 코드를 참고하여 <span className="text-green p-1 font-mono bg-[#0000004D] m-0.5 text-sm"> verify()</span> 함수를 실행하는 데 필요한 값을 채워주세요.`,
    ...
    resources: {
      ...
      derive_message_six: {
        ...
        transaction_parts_nine: `출력 #0 스크립트퍼브키(Output #0 scriptPubKey): 출력 #0 스크립트퍼브키: 해당 금액을 사용하기 위해 충족해야 하는 조건들을 정의한 스크립트입니다. 일반적으로 공개키와 여러 오퍼코드(opcode)로 구성됩니다.`,
        transaction_parts_ten: `출력 #1 금액(Output #1 value): 두 번째 출력에서 보내는 금액입니다. 마찬가지로 리틀 엔디언 형식의 정수로 표시됩니다.`,
        transaction_parts_eleven: `출력 #1 스크립트퍼브키(Output #1 scriptPubKey): 두 번째 출력 금액을 사용하기 위해 충족해야 할 조건들을 정의합니다. 보통 공개키와 다양한 오퍼코드로 구성됩니다.`,
  ...
  chapter_six: {
    title: `열쇠를 쥔 자`,
    paragraph_one: `아아악! 당신 때문에 사기를 들킨 반더풀이 당신에게 복수하려고 눈이 돌아갔습니다. 하지만 진실이 드러났음에도 불구하고, 여전히 많은 사람들이 반더풀이 스스로 만들어낸 가짜 신화(자신과 가족, 그리고 조상에 대한 이야기)에 매달리고 있습니다. 세상이 불안하고 무서운 시기일수록, 사람들은 영웅을 갈망하거든요. 그리고 안타깝게도, 많은 사람들에게 그게 바로 반더풀입니다. 그들이 가진 유일한 희망인 셈입니다.`,
    intro_one: {
      title: `들어가며`,
      ...
      paragraph_two: `당신은 잠시 멈춰 서서 지금까지의 모든 메시지가 '사토시 나카모토'라는 서명으로 끝났다는 걸 떠올립니다. 그저 비트코인의 철학을 따르는 누군가가 사용하는 가명일 거라고만 생각합니다. 설마 진짜 사토시일 리는 없다고 말입니다. 하지만... 결국 당신은 직접 물어볼 가치가 있다고 마음을 먹습니다.`,
      paragraph_three: `—”이런 질문이 좀 바보 같을 수도 있지만... 당신, 혹시 진짜 사토시인가요?”`,
    },
    intro_two: {
      title: `들어가며`,
      nav_title: `미카 3000에게 돈을 보내다`,
      paragraph_one: `—홀로캣: “요즘 너 바보처럼 들리는 말만 골라서 하더라?”`,
    ...
    in_out_one: {
      ...
      paragraph_one: `미카 3000은 반더풀의 개인 섬으로 떠나기 위한 장비를 사기 위해 1 BTC가 필요합니다. 당신은 3장에서 채굴로 얻은 보상 중 1 BTC를 그녀에게 보내기로 합니다. 그 보상은 4장에서 만든 주소로 채굴풀을 통해 입금되어 있습니다.`,
      paragraph_two: `당신은 비트코인 풀 노드를 열고, 당신의 돈이 블록체인 상에서 어디에 있는지 확인하는 명령어를 실행합니다.`,
      paragraph_three: `이건 아직 쓰이지 않은 거래 출력, 즉 UTXO 입니다. 4장에서 만든 압축 공개키 해시랑 주소가 보일 거예요. 금액은 1.61 BTC 입니다.`,
    },
    in_out_two: {
      title: `입출금의 흐름`,
      nav_title: `수신 주소`,
      ...
      paragraph_three: `이제 1.61 BTC 중 1 BTC를 이 주소로 보내는 트랜잭션을 만들고 서명해야 해요. 5장에서 사토시의 트랜잭션 구조를 살펴봤지만, 당신의 트랜잭션은 조금 다를 겁니다. 요즘은 더 새로운 방식으로 트랜잭션을 만듭니다. Segregated Witness(세그윗)라는 프로토콜을 사용해 봅시다. 이 방식은 트랜잭션 버전을 2로 설정합니다.`,
    },
    in_out_three: {
      title: `입출금의 흐름`,
      nav_title: `트랜잭션 절차`,
    ...
    in_out_four: {
      normal: {
        ...
        paragraph_eight: {
          ...
          c: ` 다만, 사용자에게 보여주거나 입력할 때만 그래요. 해시가 16진수(hex)로 제공되면, 원시 바이트로 저장하거나 전송하기 전에 바이트 순서를 뒤집어야 합니다.`,
        },
    ...
    put_it_together_one: {
      ...
      normal: {
        ...
        heading: `트랜잭션 다이제스트`,
        paragraph_one: `5장에서 우리는 트랜잭션에 서명을 하려면, 먼저 트랜잭션 데이터를 정리한 다음 해싱해서 ‘메시지’ 형태로 만들어야 한다는 걸 배웠어요. 이 메시지는 서명 알고리즘의 핵심 입력값 중 하나가 됩니다. 이제 우리는 세그윗을 사용하고 있으므로, <Link href="https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki" target="_blank" className="underline">BIP 143</Link>에 정의된 새로운 방식의 트랜잭션 다이제스트 알고리즘도 함께 구현해야 합니다.`,
    ...
    put_it_together_three: {
      normal: {
        ...
        paragraph_two: `우리의 스크립트는 하나의 Transaction 객체(설계도 기반 구조)를 만들고 여기에 서명을 해야 합니다. 이 객체에는 <span className="underline">입력</span> 하나(이전 단계에서 확인한 UTXO)와 출력 두 개가 포함됩니다.`,
        paragraph_three: `입력도 알고, 출력도 알았으니 이제 트랜잭션을 만들고 서명할 준비가 되었을까요? 아직은 아닙니다. 우리는 1.61 BTC를 입력값으로 가지고 있고, 1 BTC를 누군가에게 보내려고 합니다. 그럼 나머지 0.61 BTC는 대부분 잔돈으로 간주되어 다시 우리의 주소로 보내야 합니다.`,
        paragraph_four: `Witnesses에는 개수(count) 값이 없습니다. Witness 스택 수는 항상 입력 수와 같아야 하기 때문입니다.`,
        paragraph_five: `그리고 잊지 말아야 할 것이 있습니다. 바로 ‘수수료(fee)’입니다. 채굴자들이 우리의 트랜잭션을 블록에 포함시키도록 유도하려면, 잔돈에서 아주 조금을 수수료로 떼어내야 합니다. 예를 들어, 잔돈을 61,000,000에서 60,999,000 사토시로 줄여서 그 차이를 수수료로 쓰는 거예요.`,
  ...
  chapter_seven: {
    ...
    mempool_transaction_one: {
      ...
      paragraph_two: {
        a: `블록을 만들 땐 단순히 유효하기만 하면 안 돼요. 채굴자가 수수료 수익을 최대한 많이 얻을 수 있도록 구성해야 하죠. 그러려면 비트코인의 합의 규칙을 따라야 하고, 그 중 특히 중요한 두 가지가 있어요:`,
        b: {
          text: `트랜잭션 가중치`,
  ...
  chapter_eight: {
    ...
    intro_one: {
      title: `들어가며`,
      nav_title: `반더풀의 속임수`,
      paragraph_one: `—데보라 청크: “이 문서들은 토마스 바더풀이 했다고 주장되는 거짓말들을 역사적일 정도로 충격적으로 폭로하고 있어요. 사람들이 가장 궁금해하는 건 두 가지예요. 첫째, 이걸 혼자 해낸 건가요? 둘째, 정보를 어디서 얻었죠?”`,
      paragraph_two: `당신은 홀로캣 이야기만 이야기합니다. 정보는 사토시 나카모토라고 주장하는 누군가가 당신과 미카 3000에게 직접 전달됐다고 설명합니다. 사실 그는 해커 집단일지도 모른다고 생각합니다. 이 마지막 말에 토마스 밴더풀이 방송에 직접 전화를 걸어옵니다.`,
    },
    intro_two: {
      title: `들어가며`,
      nav_title: ` 반더풀의 분노`,
      paragraph_one: `—토마스 반더풀: “사토시 나카모토는 우리 가문이야! 방금 저 망나니가 한 말 들었어? 해커들과 한패라고 자백했잖아. 사토시 나카모토라는 이름만 도용해서 내 정당한 유산을 훔치려는 집단이지. 저 사기꾼은 블록 보상 폐지에 분노한 비트코인 지지자들의 정당한 저항을 방해하려고 거짓말을 퍼뜨리고 있어!”`,
    ...
    building_blocks_one: {
      ...
      paragraph_three: `이제 전 세계가 지켜보는 생방송 중입니다. 당신은 가장 긴 유효한 블록체인을 찾아야 하고, 그 체인을 통해 비트코인 공급이 제대로 유지되고 있다는 걸 증명해야 합니다. 그리고 이 과정에서 반더풀이 어떻게 비트코인 네트워크를 무효 블록 지뢰밭으로 만들어버렸는지도 밝힐 수 있게 될 겁니다.`,
    },
    building_blocks_two: {
      title: `기본 구성 블록`,
    ...
    building_blocks_five: {
      ...
      paragraph_two: `'inputs' 배열은 이 거래에서 ‘소모된’(즉, 없어진) 코인들이고, 'outputs' 배열은 새로 ‘생성된’ 코인들입니다. 6장에서 배운 것처럼, 거래는 항상 채굴자가 블록에 포함하도록 유도하기 위한 수수료를 지불합니다. 이 수수료는 입력 값 전체에서 출력 값 전체를 뺀 차액입니다.`,
    ...
    building_blocks_six: {
      ...
      paragraph_six: `다음 함수의 구현을 완성해보세요. 이 함수는 블록 높이를 인자로 받아, 해당 블록의 보조금 값을 사토시 단위로 반환해야 합니다.`,
    ...
    building_blocks_eight: {
      ...
      nav_title: `실전 시간!`,
      heading_one: `실전 시간!`,
      paragraph_one: `카메라가 돌아가며 전 세계 20억 명이 생중계를 지켜보고 있습니다. 다음 광고까지 몇 분밖에 안 남았습니다. 데보라 청크는 땀을 뻘뻘 흘리고 있고, 왠지 모르게 홀로캣도 땀을 흘리고 있습니다. 전화 저편 어딘가에서 반더풀도 땀을 흘리고 있겠죠. 지금이 바로 당신의 순간입니다.`,
      ...
      paragraph_three: `반더풀은 교묘합니다! 무효 블록 위에 유효 블록을 얹기도 하고, 짧은 유효 체인 위에 무효 블록을 덧씌우기도 했습니다. 완전히 미로 같은 지뢰밭입니다. 블록체인 구조를 따라가다 보면 여러 개의 유효한 가지(branch)를 동시에 추적해야 할 수도 있습니다. 유효한 부모를 가진 유효한 블록이라도, 그 체인이 가장 길지 않다면 최종 체인에는 포함되지 않을 수 있습니다. 결국엔, 가장 높은 위치에 있는 유효 블록 하나만 남게 될 겁니다.`,
      ...
      heading_three: `그리고`,
      paragraph_six: `그 블록은 반드시 다른 유효한 블록의 자식이어야 합니다. 그래야 전체적으로 유효한 체인이 되는 겁니다.`,
    ...
    outro_one: {
      ...
      nav_title: `챕터 완료`,
      heading: `생방송에서 해냈습니다!`,
      paragraph_one: `당신은 가장 긴 체인을 찾아내고, 모두에게 그 진실을 입증했습니다! 무효 블록의 목록도 놀라울 만큼 길었습니다. 반더풀은 자신이 보조금을 몰래 끼워 넣은 블록들을 사람들이 받아들이게 하려고 온갖 술수를 부렸지만, 비트코인의 규칙은 바꿀 수 없었습니다. 우리가 봤듯이, 어떤 블록이 체인에 있다고 해서 반드시 유효하다는 건 아닙니다. 이제 당신은 반더풀의 거짓말을 폭로하는 데 한 걸음 더 다가섰습니다. 말할 것도 없이, 데보라 청크의 마지막 질문에는 반더풀이 아무 대답도 하지 못했습니다.`,
    },
    resources: {
      building_blocks_three: {
  ...
  chapter_nine: {
    ...
    intro_one: {
      title: `들어가며`,
      nav_title: `협박`,
      paragraph_one: `모두가 환호하고, 모두가 웃고 있습니다. 하지만 단 한 사람, 전혀 기쁘지 않은 얼굴이 있습니다. 키가 크고 말쑥하게 차려입은 회색 머리의 남자—당신은 그를 한눈에 알아봅니다. 군중이 갈라지고, 당신은 그를 처음으로 마주하게 됩니다. 바로, 반더풀입니다!`,
      paragraph_two: `그의 주먹은 꽉 쥐어져 있습니다. 당신은 그의 사업을 망쳐놨고, 그의 가족이 수년간 벌여온 사기극을 폭로해버렸습니다. 그가 입을 열자마자 나온 첫마디는 이겁니다: “남은 평생, 매일 널 고소하겠다. 무한대로 말이지.”`,
    },
    intro_two: {
      title: `들어가며`,
      nav_title: `타협점 찾기`,
      paragraph_one: `반더풀이 소리를 지르고 있는 사이, 한 프로듀서가 다가와 전 세계 시청자들이 당신에게 비트코인과… 칼조네(*이탈리아 요리)를 기부하고 싶어 한다고 전합니다. 데보라는 당신에게 지갑 주소를 제공해달라고 요청합니다.`,
      paragraph_two: `이 말에 반더풀은 더욱 분노합니다. 기부금을 자신과 나누지 않으면, 무한대를 넘는 소송으로 당신을 질식시키겠다고 협박합니다. (그게 가능한지는 모르겠지만요.)`,
      paragraph_three: `반더풀과 기부금을 나눈다는 건 말이 안 됩니다. 이 돈은 반더풀이 비트코인을 해치려 한 걸 폭로한 당신에게 고마움을 표하고자 사람들이 보내는 겁니다. 곰곰이 생각해보니, 그는 돈이 필요한 게 아니라, 자신이 졌다는 사실이 받아들이기 힘든 것 같습니다. 순간, 그가 조금 안쓰럽게 느껴지기도 합니다. 이제 그는 완전히 막다른 골목에 몰려버렸습니다.`,
      paragraph_four: `당신의 마음을 읽기라도 한 듯, 데보라 청크가 개입합니다. 그녀는 반더풀이 일부 기부금을 보내고 싶은 자선단체가 있는지 묻습니다. 놀랍게도 반더풀은 어린이들을 돕는 비영리 단체인 '릴 비츠 재단(Lil Bits Foundation)'을 열정적으로 말합니다.`,
      paragraph_five: `반더풀의 얼굴빛은 허옇게 질렸다가 붉어지고, 다시 진한 자줏빛으로 가라앉습니다. 당신은 그와 함께 비트코인 스크립트 계약을 작성하기 시작합니다. 이 계약에서 반더풀은 릴 비츠 재단에 자금을 전달하는 책임을 맡게 되고, 당신과 반더풀이 서로를 신뢰하지 않더라도, 합의된 방식과 시점에 따라 둘 다 이 주소에서 자금을 사용할 수 있도록 설정됩니다.`,
    },
    opcodes_one: {
      title: `연산 코드 (OpCodes)`,
      nav_title: `비트코인 스크립트`,
      heading: `비트코인 스크립트`,
      paragraph_one: `챕터6에서 비트코인 스크립트에 대해 잠깐 언급했었습니다. 당시에는 별도로 깊이 들어가지 않았는데, 그 이유는 당신이 사용했던 코인들이 아주 단순한 구조로 잠겨 있었기 때문입니다—하나의 서명만 있으면 되고, 그걸 공개키로 검증하는 기본적인 스크립트였죠. 그런데 이제부터는 좀 더 흥미로운 이야기가 펼쳐질 겁니다.`,
      paragraph_two: `비트코인 출력(output)을 사용하는 데에는 두 가지 중요한 요소가 있습니다: 스크립트와 스택입니다.`,
    ...
    opcodes_two: {
      ...
      paragraph_five: `비트코인 트랜잭션 출력을 사용할 때, 필요한 정보들은 입력의 witness 섹션에 들어 있습니다 (6챕터에서 봤죠). 이 정보들은 스크립트가 실행되기 전에 스택에 먼저 푸시됩니다. 우리는 이걸 “초기 스택(INITIAL STACK)”이라고 부릅니다. 왜 중요하냐고요? 이게 바로 스크립트를 해제해서, 코인을 쓸 수 있게 해주는 ‘열쇠’ 역할을 하기 때문입니다!`,
    },
    opcodes_three: {
      title: `연산 코드 (OpCodes)`,
      nav_title: `OpCodes`,
      heading: `OpCodes`,
      paragraph_one: `스크립트는 일련의 명령어들이 순차적으로 실행되며, 스택 위의 데이터들을 조작하는 구조입니다. 스크립트 실행이 끝나면, 스택에는 '정확히 하나의 0이 아닌 값(참 값)'만 남아 있어야 합니다. 그렇지 않으면 스크립트 전체가 무효로 간주되고, 비트코인 거래도 무효가 됩니다. 비트코인 스크립트 언어에는 100개가 넘는 명령어, 즉 “연산 코드(opcode)”가 있습니다. 이번 챌린지에서는 그중 일부만 사용해볼 겁니다.`,
    ...
    opcodes_six: {
      ...
      paragraph_two: `스크립트에서 코인을 사용하기 위해 필요한 초기 스택 값을 제공하세요.`,
    },
    opcodes_seven: {
      title: `연산 코드 (OpCodes)`,
      nav_title: `멀티시그`,
      heading: `멀티시그`,
      paragraph_one: `멀티시그(Multisignature) 방식은 여러 개의 공개키 목록과, 유효한 지출을 위해 필요한 서명 수를 지정하는 정책입니다.흔히 "m-of-n" 방식이라고 부르며, n개의 공개키 중 m개의 서명이 필요하다는 뜻입니다. m, n, 그리고 공개키 목록은 일반적으로 락킹 스크립트에 포함되고, 송금자는 필요한 개수만큼의 올바른 서명만 제공하면 됩니다.`,
    ...
    opcodes_ten: {
      ...
      paragraph_three: `<span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_IF</span> 는 스택에서 값 하나를 꺼내서 불리언(boolean) 값으로 평가합니다. 만약 참(true)이면 <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_ELSE</span> 부터 <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_ENDIF</span> 까지는 건너뜁니다. 반대로 거짓(false)이면, <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_ELSE</span>까지 건너 뛰고, <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_ENDIF</span>까지 실행합니다. 조건문은 중첩(nested)할 수 있지만, 각 <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_IF</span>에는 반드시 짝이 되는 <span className="text-[#3DCFEF] rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">OP_ENDIF</span>가 있어야 합니다. 그렇지 않으면 스크립트 해석기에서 에러가 발생하고 전체 실행이 실패합니다.`,
    ...
    proposal_one: {
      ...
      paragraph_two: `각 계약 제안을 검토하면서, 비트코인 스크립트 하나와 유효한 지불 스택(spending stack) 하나씩 만들어봅시다.`,
    },
    proposal_two: {
      title: `2 of 2 멀티시그`,
      nav_title: `협력 서명`,
      heading: `2 of 2 멀티시그`,
      paragraph_one: `반더풀은 첫 번째로 2-of-2 멀티시그를 제안합니다. 기부금은 당신과 릴 비츠 재단이 50:50으로 나눠 가지게 되고, 이 주소에서 자금을 출금하려면 반드시 두 사람 모두 서명해야 합니다. 즉, 기부 주소에서 돈을 인출하려면 항상 둘이 합의해야 합니다.`,
    ...
    proposal_three: {
      ...
      paragraph_one: `잠깐만, 가만히 생각해 보니… 그 사람하고 평생 엮이고 싶진 않습니다! 그래서 새 제안을 합니다: 지금부터 2시간 동안 방송이 나가는 동안 들어오는 기부금은 전부 당신이 받고, 이후에 들어오는 기부금은 릴 비츠 재단이 가져가게 됩니다. 스튜디오 벽에 걸린 블록 타이머를 보니, 블록 높이 6930300쯤이 2시간 후에 생성될 것으로 보입니다. 그걸 기준으로 삼기로 합니다.`,
    ...
    proposal_four: {
      ...
      paragraph_one: `반더풀이 또다시 마음을 바꿨어요. 예측할 수 없는 상황은 싫다며, 이번에는 이런 조건을 제시했죠: 총 기부금 중 처음 1.0 BTC는 릴 비츠 재단이 받고, 그 이후에 들어오는 기부금은 당신이 쓰도록 합니다.`,
      paragraph_two: `비트코인 스크립트는 여러 UTXO의 총합을 확인할 수 없습니다. 그래서 중립적인 제3자인 데버라 청크를 오라클로 두기로 합의했습니다. 그녀는 블록체인에서 누적 기부금이 1.0 BTC에 도달했는지를 직접 모니터링하고, TV 생방송 중에 특정 해시 값의 프리이미지(원래 값)를 공개할 겁니다. 당신은 이 해시 값에 대한 커밋먼트를 스크립트에 넣어두고, 그녀가 프리이미지를 밝히는 순간부터 코인을 사용할 수 있습니다.`,
      paragraph_three: {
        a: `그녀는 비공개로 보안 랜덤 난수를 하나 만든 다음, 해시 다이제스트를 당신께 전달했습니다: <span className="text-[#3DCFEF] w-fit rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-sm">HASH256(FD3771E8)</span>. 이 비밀 값이 공개되기 전까지는, 절대 코인에 손댈 수 없습니다.`,
        b: ` 단 1초라도요!`,
      },
    ...
    outro_one: {
      ...
      nav_title: `쳅터 완료`,
      heading: `잘 해냈습니다!`,
      paragraph_one: `기부금은 이제 당신과 릴 비츠 재단에 나뉘어 전달되었고, 많은 시청자들이 반더풀의 행동을 밝혀준 당신에게 감사를 전하고 있습니다. 비트코인의 탈중앙화 특성 덕분에 네트워크를 장악하긴 어렵지만, 반더풀 같은 이들이 시도조차 못 하게 만들 순 없습니다. 이 모든 노력을 거쳐 마침내 진실이 드러났다는 사실에 당신도 안심합니다.`,
    },
  ...
  스택 힌트: 스크립트를 만족시키기 위해 두 개의 서명을 모두 제공해야 합니다. 스택에 들어가는 0은 CHECKMULTISIG 연산 코드의 특이한 동작을 반영한 것입니다.`,
      },
      proposal_three: {
  ...
  chapter_ten: {
    ...
    intro_one: {
      ...
      paragraph_five: {
        a: `—미카 3000: “1만 비트코인이라고? 잠깐만… 혹시 ‘라즐로’라는 이름도 그 라즐로의 후손이라서야`,
        b: `...?"`,
      },
    ...
    intro_two: {
      ...
      paragraph_two: `미카 3000과 라즐로는 태양광으로 충전되는 ePhone 인피니티를 꺼내 버튼을 두드립니다.`,
      paragraph_three: `—미카 3000: “어이쿠. 너 아까 벌인 일 때문에 채굴 수수료가 장난 아니게 올랐어. 비트코인 네트워크에 트랜잭션이 몰리면서 수수료가 천정부지로 치솟고 있어! 이건 온체인 결제 말고 오프체인으로 처리해야겠어.”`,
    },
    intro_three: {
      title: `오프체인?`,
      nav_title: `오프체인이라고?`,
      ...
      paragraph_two: `그런데도 어떻게 사람들은 매일 비트코인을 실생활에서 쓸 수 있을까요? 해답은 ‘오프체인 결제’입니다. 이건 당신도 수없이 봐왔을 거예요 — 주황색 택시가 손님을 내려주고 쌩 하고 달려가는 장면, 누군가가 할랄 푸드 카트에서 저녁을 사는 모습, 어떤 아이가 홀로독이랑 디지밀크를 들고 가게에서 나오는 장면 등. 돈이 쓰이는 방식은 정말 무한하니까요.`,
      paragraph_three: `비트코인이 본래부터 이렇게 많은 활동을 자체적으로 처리하도록 설계된 건 아니지만, 오프체인 결제를 이용하면 그게 가능해집니다.`,
      paragraph_four: `—당신: “그 정도쯤이야, 미카 3000. 문제 없어. 라즐로한테 라이트닝 레모네이드를 오프체인 결제로 사보이면 되잖아.”`,
    },
    ...
    opening_a_channel_two: {
      ...
      nav_title: `오프체인 거래`,
      heading_one: `오프체인 거래`,
      paragraph_one: `이번엔 유효한 비트코인 트랜잭션을 만들어봅시다. 다만 네트워크에 브로드캐스트하지 않고, 바로 라즐로에게 건네줄 거예요. 그러면 그는 그 대가로 당신에게 라이트닝 레모네이드를 줄 거고요. 왜냐하면 이 트랜잭션은 그가 언제든지 네트워크에 브로드캐스트해서 블록에 포함시키고, 확정되면 돈을 받을 수 있기 때문입니다.`,
      ...
      off_chain_list_two: `채굴자 수수료를 반영하세요: 라즐로가 이 트랜잭션으로 돈을 받으려면 네트워크에 브로드캐스트해야 해요. 그러려면 마이너 수수료가 필요하니, 1000 사토시를 따로 떼어 두고 거스름돈에서 그만큼 빼 주세요.`,
      ...
      heading_three: `힌트`,
      hint_one: `라즐로는 출력 0을 <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base whitespace-nowrap"> SIG(LASZLO)</span>로 소비합니다.`,
      hint_two: `당신은 출력 1을 <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base whitespace-nowrap"> SIG(YOU)</span>로 소비합니다.`,
    ...
    opening_a_channel_five: {
      ...
      paragraph_one: `—라즐로: “좋아, 고마워. 이건 좋은 시작이야. 그런데 내가 자리를 뜨고, 다시는 날 못 보게 되면 어쩔 건데? 내가 그 트랜잭션을 브로드캐스트해버리면, 네 100,000 사토시는 2-키 멀티시그 안에 갇혀서, 넌 다시는 되찾지 못할 수도 있어.”`,
    ...
    updating_the_state_one: {
      ...
      heading_three: `힌트`,
      hint_one: `출력 0은 당신이 <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base whitespace-nowrap">SIG(YOU)</span>로 소비할 수 있습니다.`,
    },
    ...
    updating_the_state_three: {
      ...
      paragraph_two: `넌 새로운 키조합 하나를 생성합니다. 개인 키는 <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base">REVOCATION_YOU_1</span> 이며, 공개 키는 <span className="rounded-sm px-1.5 py-1 h-[28px] font-mono bg-[#0000004D] m-0.5 text-base">PUBKEY(REVOCATION_YOU_1)</span> 입니다. 이런 식으로, 결제 채널 상태를 업데이트할 때마다 새로운 키조합을 생성하게 될 겁니다.`,
      heading_two: `안내`,
      revocation_list_one: `스크립트에 IF 조건문을 넣어, 라즐로가 자기 키와 철회용 키 둘 다 가지고 있어야만 이 출력을 사용할 수 있게 해보세요. (이 철회용 키는 실제 철회 시점까지 혼자 비밀로 간직하고 있어야 합니다!)`,
    ...
    updating_the_state_four: {
      ...
      paragraph_one: `—라즐로: “음, 이건 확실히 좋아졌는데... 방금 떠올랐어. 내가 철회용 키를 갖고 있어도, 결국 이 출력을 누가 먼저 쓰느냐는 ‘속도 싸움’이 될 수 있단 말이지.`,
    ...
    making_a_payment_one: {
      ...
      paragraph_three: `펀딩 트랜잭션의 출력을 소비하는 오프라인 환불 트랜잭션은 <span className="font-bold">커밋먼트 트랜잭션</span>이라고 합니다. 이게 블록체인에 확정되면 채널이 <span className="font-bold">닫히게</span> 됩니다. 첫 번째 커밋먼트 트랜잭션은 아직 라즐로에게 아무것도 결제하지 않았기 때문에 당신이 전액 환불받는 구조입니다.`,
    ...
    making_a_payment_three: {
      ...
      paragraph_two: `—라즐로: “잠깐만. 네가 이 트랜잭션에 서명하지 않으면 난 아무것도 없어. 넌 이 라이트닝 레모네이드를 마시고 사라질 수도 있는데, 나는 온체인에서 아무것도 증명할 수 없어. 우리 둘 다 트랜잭션 사본을 가지게, 네가 먼저 서명한 다음 나한테 보내는 게 어떨까?”`,
    },
    ...
    making_a_payment_six: {
      ...
      paragraph_two: `이 100,000 사토시가 잠겨 있는 2-of-2 멀티시그 출력은, 여러 개의 오프체인 트랜잭션으로 소비될 수 있어요:`,
    ...
    making_a_payment_eight: {
      ...
      paragraph_one: `밤은 길고, 오늘은 버짓콥터를 타고 집에 갈 계획도 없습니다. 라이트닝 레모네이드 한 잔 더 마실 시간, 충분합니다! 이제 당신과 라즐로는 비대칭 트랜잭션 방식도 익혔고, 철회 키를 활용한 흐름도 제대로 파악했으니, 이 프로토콜을 다시 한번 실행해봅시다.`,
    ...
    outro_two: {
      title: `마무리`,
      nav_title: `집으로 가는 길`,
      paragraph_one: `길고도 기묘했던 하루가 끝나고, 당신은 음료값을 치르고 집으로 향합니다. 그런데 3D 프린터로 지은 당신의 방갈로 문을 여는 순간—세상에, 또 반더풀이잖아요! 이게 진짜 일일까요, 아니면 밈풀 마가리타를 너무 많이 마신 탓일까요? 사방을 둘러싼 홀로캣 무리를 대동하고 있음에도, 그는 싸우러 온 건 아닌 듯합니다.`,
      paragraph_two: `—반더풀: “우린 사실 잘 아는 사이도 아니지만, 우리 할아버지 말로는 낯선 이들의 운명도 종종 얽혀있다더군요. 내 얘기를 들어줘야 해. 사토시 나카모토에 관한 모든 건… 우리 가족의 사업을 구하려던 내 마지막 몸부림이었어. 채굴이 우리에게 얼마나 큰 의미였는지 너는 상상도 못 할 거야. 나는 그저 변화가 두려웠고, 모든 걸 예전 그대로 두고 싶었던 것뿐이야.”`,
    },
    ...
    outro_three: {
      ...
      paragraph_two: `—반더풀: “비트코인에서 사토시 나카모토 다음으로 큰 이름은 비트레이랑 반더풀뿐이거든. 언젠가 누군가가 사토시 나카모토라는 이름으로 내게 말하더군. 비트코인은 이미 100년도 전에 창시자의 손을 떠났다고. 그래서 반더풀 가문의 이름을 살리기 위해, 나는 더 강력한 이름을 차지하려 했던 거야. 새 코인을 찍어낼 때의 전율, 너는 그걸 몰라. 그건 나에게 마법이었고, 지금도 그래.”`,
      paragraph_three: `—반더풀: “이제는 알아. 내가 설령 사토시의 증손자라고 해도, 비트코인을 움직이는 건 리더가 아니란 걸. 비트코인을 움직이는 건 리더가 아니야. 비트코인을 정의하는 건 바로 커뮤니티지. 심지어 사토시조차 지금의 비트코인을 바꿀 수 없어. 그게 바로 비트코인의 위대함이고, 수많은 다른 코인들이 비트코인을 따라하려다 실패한 이유야.”`,
    },
    ...
    outro_five: {
      ...
      nav_title: `해냈습니다!!!`,
      heading_one: `해냈습니다!!!`,
      paragraph_one: `사토시처럼, 당신도 처음엔 그저 무명이었지만, 당신의 아이디어에는 힘이 있었고, 그래서 사람들이 따르기 시작했죠. 사토시는 사라졌지만, 그의 정신과 철학은 여전히 계속 살아 있습니다. 이제 당신은 비트코인 개발 기술, 겸손한 태도, 그리고 포기하지 않는 끈기를 갖추게 되었고—이제 당신은 이 우아한 시스템에 의미 있는 흔적을 남길 준비가 되었습니다.  100년 넘게 전 세계를 자유롭고, 검열 없는 화폐로 연결해온 이 시스템은 앞으로도 새로운 모험을 기다리고 있죠.  비트코인이 계속 살아남을 수 있는 이유—그건 바로 당신처럼 무언가를 기꺼이 보태려는 사람들이 있기 때문입니다.  그래서 우리는 모두, 사토시입니다.`,
    },
    ...
    tab_data: {
      ...
      refund_1: `이 트랜잭션은 라즐로가 사라지더라도 자금이 사라지지 않도록 보장합니다. 그리고 당신이 라이트닝 레모네이드를 결제한 뒤에도 이 트랜잭션을 방송하지 않겠다는 신뢰를 라즐로에게 줍니다.`,
  ...
  chapter: {
    ...
    coming_soon: `곧 업데이트됩니다. 조금만 기다려 주세요.`,
    description: `사토시의 미스터리를 탐험하며 비트코인에 대해 배워보세요.`,
  },

  hero: {
    title: `Saving Satoshi`,
    description: `비트코인의 미스터리를 코드로 풀어보세요.`,
    start_journey: `시작하기`,
    tell_more: `자세히 보기`,
  },
